...
extends FileSystemProvider {
    ...
    public DirectoryStream<Path> newDirectoryStream(Path dir, final DirectoryStream.Filter<? super Path> filter) throws IOException {
        final ResourceFile.Directory lv = ResourceFileSystemProvider.toResourcePath(dir).toAbsolutePath().toDirectory();
        if (lv == null) {
    ...
    public <V extends FileAttributeView> V getFileAttributeView(Path path, Class<V> type, LinkOption ... options) {
        ResourcePath lv = ResourceFileSystemProvider.toResourcePath(path);
        if (type == BasicFileAttributeView.class) {
            return (V)lv.getAttributeView();
        }
        return null;
    }
    ...
    public <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> type, LinkOption ... options) throws IOException {
        ResourcePath lv = ResourceFileSystemProvider.toResourcePath(path).toAbsolutePath();
        if (type == BasicFileAttributes.class) {
            return (A)lv.getAttributes();
        }
