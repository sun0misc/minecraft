...
public class SignText {
    ...
    public SignText withMessage(int line, Text message, Text filteredMessage) {
        Text[] lvs = Arrays.copyOf(this.messages, this.messages.length);
        Text[] lvs2 = Arrays.copyOf(this.filteredMessages, this.filteredMessages.length);
        lvs[line] = message;
    ...
    public boolean hasRunCommandClickEvent(PlayerEntity player) {
        for (Text lv : this.getMessages(player.shouldFilterText())) {
