...
public class DoubleBlockProperties {
    public static <S extends BlockEntity> PropertySource<S> toPropertySource(BlockEntityType<S> blockEntityType, Function<BlockState, Type> typeMapper, Function<BlockState, Direction> directionMapper, DirectionProperty directionProperty, BlockState state, WorldAccess world, BlockPos pos, BiPredicate<WorldAccess, BlockPos> fallbackTester) {
        Type lv5;
        boolean bl2;
        ...
        BlockPos lv3 = pos.offset(directionMapper.apply(state));
        BlockState lv4 = world.getBlockState(lv3);
        if (lv4.isOf(state.getBlock()) && (lv5 = typeMapper.apply(lv4)) != Type.SINGLE && lv2 != lv5 && lv4.get(directionProperty) == state.get(directionProperty)) {
