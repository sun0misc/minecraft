...
public class RedstoneWireBlock extends Block {
   ...
   private static final Map DIRECTION_TO_SIDE_SHAPE;
   private static final Map DIRECTION_TO_UP_SHAPE;
   ...
   private VoxelShape getShapeForState(BlockState state) {
      ...
      while(var3.hasNext()) {
         Direction lv2 = (Direction)var3.next();
         WireConnection lv3 = (WireConnection)state.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv2));
         if (lv3 == WireConnection.SIDE) {
            lv = VoxelShapes.union(lv, (VoxelShape)DIRECTION_TO_SIDE_SHAPE.get(lv2));
         } else if (lv3 == WireConnection.UP) {
            lv = VoxelShapes.union(lv, (VoxelShape)DIRECTION_TO_UP_SHAPE.get(lv2));
         }
      }
   ...
   private BlockState getDefaultWireState(BlockView world, BlockState state, BlockPos pos) {
      ...
      while(var5.hasNext()) {
         ...
         if (!((WireConnection)state.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv))).isConnected()) {
            ...
            state = (BlockState)state.with((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv), lv2);
         }
      }
   ...
   public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
      if (direction == Direction.DOWN) {
         ...
         return lv.isConnected() == ((WireConnection)state.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction))).isConnected() && !isFullyConnected(state) ? (BlockState)state.with((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), lv) : this.getPlacementState(world, (BlockState)((BlockState)this.dotState.with(POWER, (Integer)state.get(POWER))).with((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), lv), pos);
      }
   }
   ...
   public void prepare(BlockState state, WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
      ...
      while(var7.hasNext()) {
         ...
         WireConnection lv3 = (WireConnection)state.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv2));
         if (lv3 != WireConnection.NONE && !world.getBlockState(lv.set(pos, (Direction)lv2)).isOf(this)) {
            lv.move(Direction.DOWN);
            BlockState lv4 = world.getBlockState(lv);
            if (lv4.isOf(this)) {
               BlockPos lv5 = lv.offset(lv2.getOpposite());
               world.replaceWithStateForNeighborUpdate(lv2.getOpposite(), world.getBlockState(lv5), lv, lv5, flags, maxUpdateDepth);
            }

            lv.set(pos, (Direction)lv2).move(Direction.UP);
            BlockState lv6 = world.getBlockState(lv);
            if (lv6.isOf(this)) {
               BlockPos lv7 = lv.offset(lv2.getOpposite());
               world.replaceWithStateForNeighborUpdate(lv2.getOpposite(), world.getBlockState(lv7), lv, lv7, flags, maxUpdateDepth);
            }
         }
      }
   ...
   private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
      ...
      if (bl) {
         boolean bl2 = this.canRunOnTop(world, lv, lv2);
      ...
      return !connectsTo(lv2, direction) && (lv2.isSolidBlock(world, lv) || !connectsTo(world.getBlockState(lv.down()))) ? WireConnection.NONE : WireConnection.SIDE;
   }
   ...
   public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
      ...
      if (i != 0) {
         ...
         while(var6.hasNext()) {
            Direction lv = (Direction)var6.next();
            WireConnection lv2 = (WireConnection)state.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv));
            switch (lv2) {
               case UP:
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      if (!player.getAbilities().allowModifyWorld) {
         ...
         if (isFullyConnected(state) || isNotConnected(state)) {
            ...
            lv = (BlockState)lv.with(POWER, (Integer)state.get(POWER));
   ...
   private void updateForNewState(World world, BlockPos pos, BlockState oldState, BlockState newState) {
      ...
      while(var5.hasNext()) {
         Direction lv = (Direction)var5.next();
         BlockPos lv2 = pos.offset(lv);
         if (((WireConnection)oldState.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv))).isConnected() != ((WireConnection)newState.get((Property)DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(lv))).isConnected() && world.getBlockState(lv2).isSolidBlock(world, lv2)) {
