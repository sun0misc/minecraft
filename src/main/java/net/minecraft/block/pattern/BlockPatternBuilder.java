...
public class BlockPatternBuilder {
   ...
   private final List aisles = Lists.newArrayList();
   private final Map charMap = Maps.newHashMap();
   private int height;
   private int width;

   private BlockPatternBuilder() {
      this.charMap.put(' ', (pos) -> {
         return true;
      });
   }
         ...
         if (pattern.length != this.height) {
            ...
            for(int var4 = 0; var4 < var3; ++var4) {
               ...
               if (string.length() != this.width) {
                  ...
                  throw new IllegalArgumentException("Not all rows in the given aisle are the correct width (expected " + var10002 + ", found one with " + string.length() + ")");
               }
   ...
   public BlockPatternBuilder where(char key, Predicate predicate) {
   ...
   private Predicate[][][] bakePredicates() {
      this.validate();
      Predicate[][][] predicates = (Predicate[][][])Array.newInstance(Predicate.class, new int[]{this.aisles.size(), this.height, this.width});
