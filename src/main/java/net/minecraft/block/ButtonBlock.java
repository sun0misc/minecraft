...
public class ButtonBlock extends WallMountedBlock {
   ...
   protected void tryPowerWithProjectiles(BlockState state, World world, BlockPos pos) {
      PersistentProjectileEntity lv = this.wooden ? (PersistentProjectileEntity)world.getNonSpectatingEntities(PersistentProjectileEntity.class, state.getOutlineShape(world, pos).getBoundingBox().offset(pos)).stream().findFirst().orElse((Object)null) : null;
      boolean bl = lv != null;
      ...
      if (bl != bl2) {
         ...
         world.emitGameEvent(lv, bl ? GameEvent.BLOCK_ACTIVATE : GameEvent.BLOCK_DEACTIVATE, pos);
      }
