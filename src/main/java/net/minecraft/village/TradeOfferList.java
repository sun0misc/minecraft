...
public class TradeOfferList extends ArrayList {
   ...
   public TradeOffer getValidOffer(ItemStack firstBuyItem, ItemStack secondBuyItem, int index) {
      if (index > 0 && index < this.size()) {
         ...
         return lv.matchesBuyItems(firstBuyItem, secondBuyItem) ? lv : null;
      } else {
         for(int j = 0; j < this.size(); ++j) {
            TradeOffer lv2 = (TradeOffer)this.get(j);
            if (lv2.matchesBuyItems(firstBuyItem, secondBuyItem)) {
               return lv2;
            }
         }
   ...
   public static TradeOfferList fromPacket(PacketByteBuf buf) {
      return (TradeOfferList)buf.readCollection(TradeOfferList::new, (buf2) -> {
