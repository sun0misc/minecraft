...
public class VillagerData {
   ...
   public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(Registries.VILLAGER_TYPE.getCodec().fieldOf("type").orElseGet(() -> {
         return VillagerType.PLAINS;
      }).forGetter((arg) -> {
         return arg.type;
      }), Registries.VILLAGER_PROFESSION.getCodec().fieldOf("profession").orElseGet(() -> {
         return VillagerProfession.NONE;
      }).forGetter((arg) -> {
         return arg.profession;
      }), Codec.INT.fieldOf("level").orElse(1).forGetter((arg) -> {
         return arg.level;
