...
public record TradedItem(RegistryEntry<Item> item, int count, ComponentPredicate components, ItemStack itemStack) {
    public static final Codec<TradedItem> CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)ItemStack.ITEM_CODEC.fieldOf("id")).forGetter(TradedItem::item), ((MapCodec)Codecs.POSITIVE_INT.fieldOf("count")).orElse(1).forGetter(TradedItem::count), ComponentPredicate.CODEC.optionalFieldOf("components", ComponentPredicate.EMPTY).forGetter(TradedItem::components)).apply((Applicative<TradedItem, ?>)instance, TradedItem::new));
    public static final PacketCodec<RegistryByteBuf, TradedItem> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.registryEntry(RegistryKeys.ITEM), TradedItem::item, PacketCodecs.VAR_INT, TradedItem::count, ComponentPredicate.PACKET_CODEC, TradedItem::components, TradedItem::new);
