...
public record AdvancementRequirements(List<List<String>> requirements) {
    ...
    public DataResult<AdvancementRequirements> validate(Set<String> requirements) {
        ...
        if (!requirements.equals(set2)) {
            Sets.SetView<String> set3 = Sets.difference(requirements, set2);
            Sets.SetView set4 = Sets.difference(set2, requirements);
            return DataResult.error(() -> "Advancement completion requirements did not exactly match specified criteria. Missing: " + String.valueOf(set3) + ". Unknown: " + String.valueOf(set4));
        }
