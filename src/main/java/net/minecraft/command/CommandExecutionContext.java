...
implements AutoCloseable {
    ...
    private final List<CommandQueueEntry<T>> pendingCommands = new ObjectArrayList<CommandQueueEntry<T>>();
    ...
    public void run() {
        ...
        while (true) {
            ...
            CommandQueueEntry<T> lv = this.commandQueue.pollFirst();
            if (lv == null) {
                return;
            }
            this.currentDepth = lv.frame().depth();
            lv.execute(this);
            if (this.queueOverflowed) {
