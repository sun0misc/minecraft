...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("AttributeIdFix (ItemStack)", type, typed2 -> typed2.updateTyped(opticFinder, typed -> typed.update(DSL.remainderFinder(), class_9785::method_60686))), this.fixTypeEverywhereTyped("AttributeIdFix (Entity)", this.getInputSchema().getType(TypeReferences.ENTITY), class_9785::method_60679), this.fixTypeEverywhereTyped("AttributeIdFix (Player)", this.getInputSchema().getType(TypeReferences.PLAYER), class_9785::method_60679));
    }

    private static Stream<Dynamic<?>> method_60683(Stream<?> stream) {
        return class_9785.method_60687(stream);
    }
    ...
    private static Stream<Dynamic<?>> method_60687(Stream<Dynamic<?>> stream) {
        ...
        stream.forEach(dynamic -> {
            UUID uUID = class_9785.method_60684(dynamic.get("uuid").asIntStream().toArray());
            String string = dynamic.get("name").asString("");
            String string2 = uUID != null ? field_51992.get(uUID) : null;
            String string3 = field_51993.get(string);
            if (string2 != null) {
    ...
    private static Dynamic<?> method_60680(Dynamic<?> dynamic2) {
    ...
    private static Dynamic<?> method_60686(Dynamic<?> dynamic) {
        return dynamic.update("attribute_modifiers", dynamic2 -> dynamic2.update("modifiers", dynamic -> DataFixUtils.orElse(dynamic.asStreamOpt().result().map(class_9785::method_60683).map(dynamic::createList), dynamic)));
    }

    private static Dynamic<?> method_60688(Dynamic<?> dynamic) {
        return dynamic.renameField("Name", "id").renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", dynamic2 -> DataFixUtils.orElse(dynamic2.asStreamOpt().result().map(stream -> stream.map(class_9785::method_60680)).map(class_9785::method_60683).map(dynamic::createList), dynamic2));
    }

    private static Typed<?> method_60679(Typed<?> typed) {
        return typed.update(DSL.remainderFinder(), dynamic2 -> dynamic2.renameAndFixField("Attributes", "attributes", dynamic -> DataFixUtils.orElse(dynamic.asStreamOpt().result().map(stream -> stream.map(class_9785::method_60688)).map(dynamic::createList), dynamic)));
    }
