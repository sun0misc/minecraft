...
public class LogFileCompressor {
    ...
    private LogFile get(Path path) {
        ...
        LogId lv = LogId.fromFileName(string.substring(0, i));
        if (lv != null) {
    ...
    implements Iterable<LogFile> {
        ...
        public LogFileIterable removeExpired(LocalDate currentDate, int retentionDays) {
            this.logs.removeIf(log -> {
                LogId lv = log.id();
                LocalDate localDate2 = lv.date().plusDays(retentionDays);
        ...
        public LogFileIterable compressAll() {
            ...
            while (listIterator.hasNext()) {
                LogFile lv = listIterator.next();
                try {
                    listIterator.set(lv.compress());
                } catch (IOException iOException) {
                    LOGGER.warn("Failed to compress event log file: {}", (Object)lv.path(), (Object)iOException);
                }
            }
            return this;
        }
