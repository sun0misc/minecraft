...
public class InterpolatedNoiseSampler implements DensityFunction.Base {
   ...
   private static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(SCALE_AND_FACTOR_RANGE.fieldOf("xz_scale").forGetter((arg) -> {
         return arg.xzScale;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("y_scale").forGetter((arg) -> {
         return arg.yScale;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("xz_factor").forGetter((arg) -> {
         return arg.xzFactor;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("y_factor").forGetter((arg) -> {
         return arg.yFactor;
      }), Codec.doubleRange(1.0, 8.0).fieldOf("smear_scale_multiplier").forGetter((arg) -> {
         return arg.smearScaleMultiplier;
   ...
   public double sample(DensityFunction.NoisePos pos) {
      ...
      for(int r = 0; r < 16; ++r) {
         ...
         if (!bl2) {
            lv2 = this.lowerInterpolatedNoise.getOctave(r);
            if (lv2 != null) {
         ...
         if (!bl3) {
            lv2 = this.upperInterpolatedNoise.getOctave(r);
            if (lv2 != null) {
