...
implements DensityFunction.Base {
    ...
    private static final MapCodec<InterpolatedNoiseSampler> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)SCALE_AND_FACTOR_RANGE.fieldOf("xz_scale")).forGetter(arg -> arg.xzScale), ((MapCodec)SCALE_AND_FACTOR_RANGE.fieldOf("y_scale")).forGetter(arg -> arg.yScale), ((MapCodec)SCALE_AND_FACTOR_RANGE.fieldOf("xz_factor")).forGetter(arg -> arg.xzFactor), ((MapCodec)SCALE_AND_FACTOR_RANGE.fieldOf("y_factor")).forGetter(arg -> arg.yFactor), ((MapCodec)Codec.doubleRange(1.0, 8.0).fieldOf("smear_scale_multiplier")).forGetter(arg -> arg.smearScaleMultiplier)).apply((Applicative<InterpolatedNoiseSampler, ?>)instance, InterpolatedNoiseSampler::createBase3dNoiseFunction));
    ...
    public double sample(DensityFunction.NoisePos pos) {
        ...
        double o = 1.0;
        for (int p = 0; p < 8; ++p) {
            PerlinNoiseSampler lv = this.interpolationNoise.getOctave(p);
            if (lv != null) {
                n += lv.sample(OctavePerlinNoiseSampler.maintainPrecision(g * o), OctavePerlinNoiseSampler.maintainPrecision(h * o), OctavePerlinNoiseSampler.maintainPrecision(i * o), k * o, h * o) / o;
            }
            o /= 2.0;
        }
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            double v = j * o;
            if (!bl2 && (lv2 = this.lowerInterpolatedNoise.getOctave(r)) != null) {
                l += lv2.sample(s, t, u, v, e * o) / o;
            }
            if (!bl3 && (lv2 = this.upperInterpolatedNoise.getOctave(r)) != null) {
                m += lv2.sample(s, t, u, v, e * o) / o;
            }
            o /= 2.0;
        }
