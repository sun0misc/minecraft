...
public class CommandSuggestionsS2CPacket implements Packet {
   ...
   public CommandSuggestionsS2CPacket(PacketByteBuf buf) {
      ...
      List list = buf.readList((buf2) -> {
         String string = buf2.readString();
         Text lv = (Text)buf2.readNullable(PacketByteBuf::readText);
         return new Suggestion(stringRange, string, lv);
      });
   ...
   public void write(PacketByteBuf buf) {
      ...
      buf.writeCollection(this.suggestions.getList(), (buf2, suggestion) -> {
         ...
         buf2.writeNullable(suggestion.getTooltip(), (buf3, tooltip) -> {
            buf3.writeText(Texts.toText(tooltip));
         });
      });
   }
