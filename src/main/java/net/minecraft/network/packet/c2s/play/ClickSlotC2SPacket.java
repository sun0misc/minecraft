...
public class ClickSlotC2SPacket implements Packet {
   ...
   private final Int2ObjectMap modifiedStacks;

   public ClickSlotC2SPacket(int syncId, int revision, int slot, int button, SlotActionType actionType, ItemStack stack, Int2ObjectMap modifiedStacks) {
      this.syncId = syncId;
   ...
   public ClickSlotC2SPacket(PacketByteBuf buf) {
      ...
      this.actionType = (SlotActionType)buf.readEnumConstant(SlotActionType.class);
      IntFunction intFunction = PacketByteBuf.getMaxValidator(Int2ObjectOpenHashMap::new, 128);
      this.modifiedStacks = Int2ObjectMaps.unmodifiable((Int2ObjectMap)buf.readMap(intFunction, (bufx) -> {
