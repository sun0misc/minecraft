...
AutoCloseable {
    ...
    public static Task<Runnable> createMessage(AbstractChunkHolder holder, Runnable task) {
        return ChunkTaskPrioritySystem.createMessage(task, holder.getPos().toLong(), holder::getCompletedLevel);
    }

    public static <T> Task<T> createTask(AbstractChunkHolder holder, Function<MessageListener<Unit>, T> taskFunction) {
    ...
    private <T> LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> getQueue(MessageListener<T> actor) {
        ...
        if (lv == null) {
            throw Util.throwOrPause(new IllegalArgumentException("No queue for: " + String.valueOf(actor)));
        }
        return lv;
    }
    ...
    public String getDebugString() {
        return this.queues.entrySet().stream().map(entry -> ((MessageListener)entry.getKey()).getName() + "=[" + ((LevelPrioritizedQueue)entry.getValue()).getBlockingChunks().stream().map(pos -> pos + ":" + String.valueOf(new ChunkPos((long)pos))).collect(Collectors.joining(",")) + "]").collect(Collectors.joining(",")) + ", s=" + this.idleActors.size();
    }
