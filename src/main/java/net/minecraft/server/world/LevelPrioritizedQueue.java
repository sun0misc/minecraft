...
public class LevelPrioritizedQueue {
   ...
   private final List levelToPosToElements;
   ...
   protected void updateLevel(int fromLevel, ChunkPos pos, int toLevel) {
      if (fromLevel < LEVEL_COUNT) {
         Long2ObjectLinkedOpenHashMap long2ObjectLinkedOpenHashMap = (Long2ObjectLinkedOpenHashMap)this.levelToPosToElements.get(fromLevel);
   ...
   protected void add(Optional element, long pos, int level) {
   ...
   public Stream poll() {
      if (this.blockingChunks.size() >= this.maxBlocking) {
         ...
         Long2ObjectLinkedOpenHashMap long2ObjectLinkedOpenHashMap = (Long2ObjectLinkedOpenHashMap)this.levelToPosToElements.get(i);
         long l = long2ObjectLinkedOpenHashMap.firstLongKey();

         List list;
         for(list = (List)long2ObjectLinkedOpenHashMap.removeFirst(); this.hasQueuedElement() && ((Long2ObjectLinkedOpenHashMap)this.levelToPosToElements.get(this.firstNonEmptyLevel)).isEmpty(); ++this.firstNonEmptyLevel) {
         }
