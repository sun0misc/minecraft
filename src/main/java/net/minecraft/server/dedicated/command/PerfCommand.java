...
public class PerfCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("perf").requires((source) -> {
   ...
   private static void saveReport(ServerCommandSource source, Path tempProfilingDirectory, MinecraftServer server) {
      ...
      ZipCompressor lv = new ZipCompressor(RecordDumper.DEBUG_PROFILING_DIRECTORY.resolve(string2));

      try {
         lv.write(Paths.get("system.txt"), server.addSystemDetails(new SystemDetails()).collect());
         lv.copyAll(tempProfilingDirectory);
