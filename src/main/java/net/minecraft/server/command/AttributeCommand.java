...
public class AttributeCommand {
    ...
    private static int executeModifierValueGet(ServerCommandSource source, Entity target, RegistryEntry<EntityAttribute> attribute, Identifier arg4, double multiplier) throws CommandSyntaxException {
        LivingEntity lv = AttributeCommand.getLivingEntityWithAttribute(target, attribute);
        AttributeContainer lv2 = lv.getAttributes();
        if (!lv2.hasModifierForAttribute(attribute, arg4)) {
            throw NO_MODIFIER_EXCEPTION.create(target.getName(), AttributeCommand.getName(attribute), arg4);
        }
        double e = lv2.getModifierValue(attribute, arg4);
        source.sendFeedback(() -> Text.translatable("commands.attribute.modifier.value.get.success", Text.of(arg4), AttributeCommand.getName(attribute), target.getName(), e), false);
        return (int)(e * multiplier);
    }
    ...
    private static int executeModifierAdd(ServerCommandSource source, Entity target, RegistryEntry<EntityAttribute> attribute, Identifier arg4, double d, EntityAttributeModifier.Operation arg5) throws CommandSyntaxException {
        EntityAttributeInstance lv = AttributeCommand.getAttributeInstance(target, attribute);
        EntityAttributeModifier lv2 = new EntityAttributeModifier(arg4, d, arg5);
        if (lv.hasModifier(arg4)) {
            throw MODIFIER_ALREADY_PRESENT_EXCEPTION.create(target.getName(), AttributeCommand.getName(attribute), arg4);
        }
        lv.addPersistentModifier(lv2);
        source.sendFeedback(() -> Text.translatable("commands.attribute.modifier.add.success", Text.of(arg4), AttributeCommand.getName(attribute), target.getName()), false);
        return 1;
    }

    private static int executeModifierRemove(ServerCommandSource source, Entity target, RegistryEntry<EntityAttribute> attribute, Identifier arg4) throws CommandSyntaxException {
        EntityAttributeInstance lv = AttributeCommand.getAttributeInstance(target, attribute);
        if (lv.tryRemoveModifier(arg4)) {
            source.sendFeedback(() -> Text.translatable("commands.attribute.modifier.remove.success", Text.of(arg4), AttributeCommand.getName(attribute), target.getName()), false);
            return 1;
        }
        throw NO_MODIFIER_EXCEPTION.create(target.getName(), AttributeCommand.getName(attribute), arg4);
    }
