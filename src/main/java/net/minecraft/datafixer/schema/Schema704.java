...
public class Schema704 extends Schema {
   ...
   public Map registerBlockEntities(Schema schema) {
      ...
      schema.register(map, "minecraft:flower_pot", (name) -> {
   ...
   public void registerTypes(Schema schema, Map entityTypes, Map blockEntityTypes) {
      ...
      schema.registerType(false, TypeReferences.BLOCK_ENTITY, () -> {
         return DSL.taggedChoiceLazy("id", IdentifierNormalizingSchema.getIdentifierType(), blockEntityTypes);
      });
      schema.registerType(true, TypeReferences.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", TypeReferences.ITEM_NAME.in(schema), "tag", DSL.optionalFields("EntityTag", TypeReferences.ENTITY_TREE.in(schema), "BlockEntityTag", TypeReferences.BLOCK_ENTITY.in(schema), "CanDestroy", DSL.list(TypeReferences.BLOCK_NAME.in(schema)), "CanPlaceOn", DSL.list(TypeReferences.BLOCK_NAME.in(schema)), "Items", DSL.list(TypeReferences.ITEM_STACK.in(schema)))), field_5745, HookFunction.IDENTITY);
      });
   }
}
