...
extends DataFix {
    ...
    private static <T> Optional<Dynamic<T>> updateId(Dynamic<T> dynamic, String idKey) {
        return dynamic.get(idKey).asNumber().result().map(oldId -> (String)OLD_TO_NEW_IDS.get(oldId.intValue())).map(dynamic::createString);
    }
    ...
    private static <T> Dynamic<T> fixEffectList(Dynamic<T> dynamic, String oldEffectListKey, String newEffectListKey) {
        Optional<Dynamic> optional = dynamic.get(oldEffectListKey).asStreamOpt().result().map(stream -> dynamic.createList(stream.map(StatusEffectFix::fixEffect)));
    ...
    private static <T> Dynamic<T> method_53083(Dynamic<T> dynamic, Dynamic<T> dynamic2) {
        dynamic2 = StatusEffectFix.renameKeyAndUpdateId(dynamic, "EffectId", dynamic2, "id");
    ...
    private static <T> Dynamic<T> method_53095(Dynamic<T> dynamic) {
        return StatusEffectFix.method_53083(dynamic, dynamic);
    }

    private Typed<?> method_53081(Typed<?> typed2, DSL.TypeReference typeReference, String string, Function<Dynamic<?>, Dynamic<?>> function) {
    ...
    private TypeRewriteRule makeBlockEntitiesRule() {
        ...
        return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", type, typed -> {
            typed = this.method_53081((Typed<?>)typed, TypeReferences.BLOCK_ENTITY, "minecraft:beacon", dynamic -> {
    ...
    private static <T> Dynamic<T> fixStewEffectsKey(Dynamic<T> dynamic) {
        ...
        Dynamic<T> dynamic3 = StatusEffectFix.method_53083(dynamic, dynamic2);
    ...
    private TypeRewriteRule makeEntitiesRule() {
        ...
        return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", type, typed -> {
            typed = this.method_53081((Typed<?>)typed, TypeReferences.ENTITY, "minecraft:mooshroom", StatusEffectFix::fixStewEffectsKey);
            typed = this.method_53081((Typed<?>)typed, TypeReferences.ENTITY, "minecraft:arrow", StatusEffectFix::fixCustomPotionEffectsKey);
            typed = this.method_53081((Typed<?>)typed, TypeReferences.ENTITY, "minecraft:area_effect_cloud", StatusEffectFix::fixEffectsKey);
    ...
    private static <T> Dynamic<T> method_53106(Dynamic<T> dynamic) {
        Optional<Dynamic> optional = dynamic.get("Effects").asStreamOpt().result().map(stream -> dynamic.createList(stream.map(StatusEffectFix::method_53095)));
        ...
        return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", type, typed2 -> {
            ...
            if (optional.isPresent()) {
                ...
                if (string.equals("minecraft:suspicious_stew")) {
                    return typed2.updateTyped(opticFinder2, typed -> typed.update(DSL.remainderFinder(), StatusEffectFix::method_53106));
                }
                if (POTION_ITEM_IDS.contains(string)) {
                    return typed2.updateTyped(opticFinder2, typed -> typed.update(DSL.remainderFinder(), dynamic -> StatusEffectFix.fixEffectList(dynamic, "CustomPotionEffects", "custom_potion_effects")));
                }
            }
            return typed2;
        });
