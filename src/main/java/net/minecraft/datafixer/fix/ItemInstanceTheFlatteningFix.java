...
public class ItemInstanceTheFlatteningFix extends DataFix {
   ...
   public static String getItem(@Nullable String originalName, int damage) {
      if (ORIGINAL_ITEM_NAMES.contains(originalName)) {
         String string2 = (String)FLATTENING_MAP.get(originalName + "." + damage);
         return string2 == null ? (String)FLATTENING_MAP.get(originalName + ".0") : string2;
      } else {
         return null;
      }
   }
   ...
   static {
      ORIGINAL_ITEM_NAMES = (Set)FLATTENING_MAP.keySet().stream().map((string) -> {
