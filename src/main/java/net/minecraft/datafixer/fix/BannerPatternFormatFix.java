...
extends ChoiceFix {
    ...
    private static Dynamic<?> replacePatternAndColor(Dynamic<?> dynamic2) {
        dynamic2 = dynamic2.renameAndFixField("Pattern", "pattern", dynamic -> DataFixUtils.orElse(dynamic.asString().map(string -> OLD_TO_NEW_PATTERNS.getOrDefault(string, (String)string)).map(dynamic::createString).result(), dynamic));
        dynamic2 = dynamic2.set("color", dynamic2.createString(BannerPatternFormatFix.getColorFromInt(dynamic2.get("Color").asInt(0))));
