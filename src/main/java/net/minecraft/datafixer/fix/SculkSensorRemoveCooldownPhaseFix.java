...
public class SculkSensorRemoveCooldownPhaseFix extends DataFix {
   ...
   private static Dynamic removeCooldownPhase(Dynamic dynamic) {
      ...
      return !optional.equals(Optional.of("minecraft:sculk_sensor")) && !optional.equals(Optional.of("minecraft:calibrated_sculk_sensor")) ? dynamic : dynamic.update("Properties", (dynamicx) -> {
   ...
   protected TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("sculk_sensor_remove_cooldown_phase_fix", this.getInputSchema().getType(TypeReferences.BLOCK_STATE), (typed) -> {
         return typed.update(DSL.remainderFinder(), SculkSensorRemoveCooldownPhaseFix::removeCooldownPhase);
      });
   }
}
