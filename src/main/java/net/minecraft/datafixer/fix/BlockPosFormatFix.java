...
extends DataFix {
    ...
    private <T> Dynamic<T> fixMapItemFrames(Dynamic<T> dynamic) {
        return dynamic.update("frames", dynamic2 -> dynamic2.createList(dynamic2.asStream().map(dynamic -> {
            dynamic = dynamic.renameAndFixField("Pos", "pos", FixUtil::fixBlockPos);
    ...
    public TypeRewriteRule makeRule() {
        ...
        list.add(this.fixTypeEverywhereTyped("BlockPos format for map frames", this.getInputSchema().getType(TypeReferences.SAVED_DATA_MAP_DATA), typed -> typed.update(DSL.remainderFinder(), dynamic -> dynamic.update("data", this::fixMapItemFrames))));
        Type<?> type = this.getInputSchema().getType(TypeReferences.ITEM_STACK);
        list.add(this.fixTypeEverywhereTyped("BlockPos format for compass target", type, ItemNbtFix.method_56971(type, "minecraft:compass"::equals, dynamic -> dynamic.update("LodestonePos", FixUtil::fixBlockPos))));
    ...
    private void addEntityFixes(List<TypeRewriteRule> rules) {
        ...
        rules.add(this.fixTypeEverywhereTyped("BlockPos format in Leash for mobs", this.getInputSchema().getType(TypeReferences.ENTITY), typed -> typed.update(DSL.remainderFinder(), dynamic -> dynamic.renameAndFixField("Leash", "leash", FixUtil::fixBlockPos))));
    }
