...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("Banner entity custom_name to item_name component fix", type, typed -> {
            ...
            return object.equals("minecraft:banner") ? this.fix((Typed<?>)typed, opticFinder) : typed;
        });
    }
    ...
    private Typed<?> fix(Typed<?> typed, OpticFinder<?> opticFinder) {
        ...
        if (bl) {
            Typed<?> typed2 = typed.getOrCreateTyped(opticFinder).update(DSL.remainderFinder(), dynamic -> dynamic.set("minecraft:item_name", optionalDynamic.result().get()).set("minecraft:hide_additional_tooltip", dynamic.createMap(Map.of())));
