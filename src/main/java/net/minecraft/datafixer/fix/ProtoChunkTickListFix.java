...
extends DataFix {
    ...
    private Dynamic<?> fixToBeTicked(Dynamic<?> dynamic2, Int2ObjectMap<Supplier<class_6741>> int2ObjectMap, byte b, int i2, int j, String string, Function<Dynamic<?>, String> function) {
        ...
        for (int k = 0; k < list.size(); ++k) {
            ...
            Stream<Dynamic> stream2 = ((Dynamic)list.get(k)).asStream().mapToInt(dynamic -> dynamic.asShort((short)-1)).filter(i -> i > 0).mapToObj(l -> this.method_39255(dynamic2, (Supplier)supplier, i2, l2, j, l, function));
