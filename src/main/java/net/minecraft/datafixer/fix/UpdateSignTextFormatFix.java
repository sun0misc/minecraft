...
public class UpdateSignTextFormatFix extends ChoiceFix {
   ...
   private static Dynamic updateFrontText(Dynamic dynamic, String textKey) {
      Dynamic dynamic2 = dynamic.createString(getEmptyText());
      return dynamic.createList(Stream.of((Dynamic)dynamic.get(textKey + "1").result().orElse(dynamic2), (Dynamic)dynamic.get(textKey + "2").result().orElse(dynamic2), (Dynamic)dynamic.get(textKey + "3").result().orElse(dynamic2), (Dynamic)dynamic.get(textKey + "4").result().orElse(dynamic2)));
   }

   private static Dynamic updateBackText(Dynamic dynamic) {
      Dynamic dynamic2 = dynamic.createString(getEmptyText());
