...
extends DataFix {
    ...
    private static void fixAttributeModifiers(StackData data, Dynamic<?> dynamic, int hideFlags) {
        boolean bl;
        List<Dynamic> list = data.getAndRemove("AttributeModifiers").asList(ItemStackComponentizationFix::fixAttributeModifier);
        boolean bl2 = bl = (hideFlags & 2) != 0;
        if (list.isEmpty() && !bl) {
            return;
        }
    ...
    public static Dynamic<?> createProfileDynamic(Dynamic<?> dynamic) {
        ...
        if (optional.isPresent()) {
            if (ItemStackComponentizationFix.method_58048(optional.get())) {
        ...
        Dynamic dynamic3 = dynamic.emptyMap();
        if (ItemStackComponentizationFix.method_58048(string)) {
    ...
    private static Dynamic<?> createPropertiesDynamic(OptionalDynamic<?> optionalDynamic) {
        Map<String, List> map = optionalDynamic.asMap(dynamic -> dynamic.asString(""), dynamic2 -> dynamic2.asList(dynamic -> {
