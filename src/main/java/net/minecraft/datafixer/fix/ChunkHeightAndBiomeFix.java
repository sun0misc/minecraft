...
extends DataFix {
    ...
    private static Dynamic<?>[] fixBiomes(Dynamic<?> level, boolean overworld, int i, MutableBoolean heightAlreadyUpdated) {
        ...
        if (is != null && is.length == 1536) {
            ...
            if (overworld) {
                Dynamic<?> dynamic2 = ChunkHeightAndBiomeFix.fixBiomes(level, sectionY -> ChunkHeightAndBiomeFix.getClamped(is, sectionY % 16));
                Dynamic<?> dynamic3 = ChunkHeightAndBiomeFix.fixBiomes(level, sectionY -> ChunkHeightAndBiomeFix.getClamped(is, sectionY % 16 + 1008));
                for (l = 0; l < 4; ++l) {
                    dynamics[l] = dynamic2;
                }
                for (l = 20; l < 24; ++l) {
            ...
            if (lv != null) {
                ...
                for (int i = 0; i < 16; ++i) {
                    for (int j = 0; j < 16; ++j) {
                        ...
                        boolean bl = bl6 = dynamic3 != null && "minecraft:air".equals(dynamic3.get("Name").asString(""));
                        if (bl6) {
                            bitSet.set(i * 16 + j);
                        }
                        bl4 |= bl5;
                ...
                if (bl4 && bitSet.cardinality() != bitSet.size()) {
                    ...
                    level = level.set("below_zero_retrogen", level.createMap(ImmutableMap.of(level.createString("target_status"), dynamic4, level.createString("missing_bedrock"), level.createLongList(LongStream.of(bitSet.toLongArray())))));
    ...
    private static Dynamic<?> fixCarvingMasks(Dynamic<?> level, int sectionsPerChunk, int oldBottomSectionY) {
        ...
        dynamic2 = dynamic2.updateMapValues(mask -> {
    ...
    private static Dynamic<?> fixChunkSectionList(Dynamic<?> level, String key) {
        ...
        Dynamic dynamic2 = level.emptyList();
        for (int i = 0; i < 4; ++i) {
            list.add(0, dynamic2);
            list.add(dynamic2);
        }
    ...
    private static Dynamic<?> fixBiomes(Dynamic<?> level, Int2IntFunction biomeGetter) {
        ...
        Dynamic dynamic2 = level.createList(int2IntMap.keySet().stream().map(rawBiomeId -> level.createString(RAW_BIOME_IDS.getOrDefault((int)rawBiomeId, PLAINS_ID))));
