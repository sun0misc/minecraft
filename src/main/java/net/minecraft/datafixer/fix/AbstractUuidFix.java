...
extends DataFix {
    ...
    protected static Optional<Dynamic<?>> updateRegularMostLeast(Dynamic<?> dynamic, String oldKey, String newKey) {
        ...
        return AbstractUuidFix.createArrayFromMostLeastTags(dynamic, string3, string4).map(dynamic2 -> dynamic.remove(string3).remove(string4).set(newKey, (Dynamic<?>)dynamic2));
    }
