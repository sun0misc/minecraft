...
public class Stats {
    ...
    private static Identifier register(String id, StatFormatter formatter) {
        Identifier lv = Identifier.method_60656(id);
        Registry.register(Registries.CUSTOM_STAT, id, lv);
        CUSTOM.getOrCreateStat(lv, formatter);
        return lv;
    }

    private static <T> StatType<T> registerType(String id, Registry<T> registry) {
        MutableText lv = Text.translatable("stat_type.minecraft." + id);
        return Registry.register(Registries.STAT_TYPE, id, new StatType<T>(registry, lv));
    }
}
