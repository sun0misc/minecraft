...
public class ItemGroups {
    ...
    private static final RegistryKey<ItemGroup> BUILDING_BLOCKS = ItemGroups.register("building_blocks");
    private static final RegistryKey<ItemGroup> COLORED_BLOCKS = ItemGroups.register("colored_blocks");
    private static final RegistryKey<ItemGroup> NATURAL = ItemGroups.register("natural_blocks");
    private static final RegistryKey<ItemGroup> FUNCTIONAL = ItemGroups.register("functional_blocks");
    private static final RegistryKey<ItemGroup> REDSTONE = ItemGroups.register("redstone_blocks");
    private static final RegistryKey<ItemGroup> HOTBAR = ItemGroups.register("hotbar");
    private static final RegistryKey<ItemGroup> SEARCH = ItemGroups.register("search");
    private static final RegistryKey<ItemGroup> TOOLS = ItemGroups.register("tools_and_utilities");
    private static final RegistryKey<ItemGroup> COMBAT = ItemGroups.register("combat");
    private static final RegistryKey<ItemGroup> FOOD_AND_DRINK = ItemGroups.register("food_and_drinks");
    private static final RegistryKey<ItemGroup> INGREDIENTS = ItemGroups.register("ingredients");
    private static final RegistryKey<ItemGroup> SPAWN_EGGS = ItemGroups.register("spawn_eggs");
    private static final RegistryKey<ItemGroup> OPERATOR = ItemGroups.register("op_blocks");
    private static final RegistryKey<ItemGroup> INVENTORY = ItemGroups.register("inventory");
    ...
    public static ItemGroup registerAndGetDefault(Registry<ItemGroup> registry) {
        ...
        Registry.register(registry, FUNCTIONAL, ItemGroup.create(ItemGroup.Row.TOP, 3).displayName(Text.translatable("itemGroup.functional")).icon(() -> new ItemStack(Items.OAK_SIGN)).entries((displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.PAINTING_VARIANT).ifPresent(arg3 -> ItemGroups.addPaintings(entries, displayContext.lookup(), arg3, registryEntry -> registryEntry.isIn(PaintingVariantTags.PLACEABLE), ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS));
        ...
        Registry.register(registry, SEARCH, ItemGroup.create(ItemGroup.Row.TOP, 6).displayName(Text.translatable("itemGroup.search")).icon(() -> new ItemStack(Items.COMPASS)).entries((displayContext, entries) -> {
            ...
            for (ItemGroup lv : registry) {
                if (lv.getType() == ItemGroup.Type.SEARCH) continue;
                set.addAll(lv.getSearchTabStacks());
            }
            entries.addAll(set);
        }).texture(field_52023).special().type(ItemGroup.Type.SEARCH).build());
        ...
        Registry.register(registry, COMBAT, ItemGroup.create(ItemGroup.Row.BOTTOM, 1).displayName(Text.translatable("itemGroup.combat")).icon(() -> new ItemStack(Items.NETHERITE_SWORD)).entries((displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.POTION).ifPresent(arg3 -> ItemGroups.addPotions(entries, arg3, Items.TIPPED_ARROW, ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS, displayContext.enabledFeatures()));
        }).build());
        ...
        Registry.register(registry, INGREDIENTS, ItemGroup.create(ItemGroup.Row.BOTTOM, 3).displayName(Text.translatable("itemGroup.ingredients")).icon(() -> new ItemStack(Items.IRON_INGOT)).entries((displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.ENCHANTMENT).ifPresent(arg2 -> {
                ItemGroups.addMaxLevelEnchantedBooks(entries, arg2, ItemGroup.StackVisibility.PARENT_TAB_ONLY);
                ItemGroups.addAllLevelEnchantedBooks(entries, arg2, ItemGroup.StackVisibility.SEARCH_TAB_ONLY);
            });
        }).build());
        Registry.register(registry, SPAWN_EGGS, ItemGroup.create(ItemGroup.Row.BOTTOM, 4).displayName(Text.translatable("itemGroup.spawnEggs")).icon(() -> new ItemStack(Items.PIG_SPAWN_EGG)).entries((displayContext, entries) -> {
        ...
        return Registry.register(registry, INVENTORY, ItemGroup.create(ItemGroup.Row.BOTTOM, 6).displayName(Text.translatable("itemGroup.inventory")).icon(() -> new ItemStack(Blocks.CHEST)).texture(field_52022).noRenderedName().special().type(ItemGroup.Type.INVENTORY).noScrollbar().build());
    }

    public static void collect() {
        HashMap<Pair<ItemGroup.Row, Integer>, String> map = new HashMap<Pair<ItemGroup.Row, Integer>, String>();
        for (RegistryKey<ItemGroup> lv : Registries.ITEM_GROUP.getKeys()) {
            ItemGroup lv2 = Registries.ITEM_GROUP.getOrThrow(lv);
            String string = lv2.getDisplayName().getString();
            String string2 = map.put(Pair.of(lv2.getRow(), lv2.getColumn()), string);
    ...
    private static void addMaxLevelEnchantedBooks(ItemGroup.Entries entries, RegistryWrapper<Enchantment> registryWrapper, ItemGroup.StackVisibility arg3) {
        registryWrapper.streamEntries().map(arg -> EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry((RegistryEntry<Enchantment>)arg, ((Enchantment)arg.value()).getMaxLevel()))).forEach(stack -> entries.add((ItemStack)stack, arg3));
    }

    private static void addAllLevelEnchantedBooks(ItemGroup.Entries entries, RegistryWrapper<Enchantment> registryWrapper, ItemGroup.StackVisibility arg3) {
        registryWrapper.streamEntries().flatMap(arg -> IntStream.rangeClosed(((Enchantment)arg.value()).getMinLevel(), ((Enchantment)arg.value()).getMaxLevel()).mapToObj(i -> EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry((RegistryEntry<Enchantment>)arg, i)))).forEach(stack -> entries.add((ItemStack)stack, arg3));
    }
    ...
    private static void addSuspiciousStews(ItemGroup.Entries entries, ItemGroup.StackVisibility visibility) {
        ...
        Set<ItemStack> set = ItemStackSet.create();
        for (SuspiciousStewIngredient lv : list) {
            ItemStack lv2 = new ItemStack(Items.SUSPICIOUS_STEW);
            lv2.set(DataComponentTypes.SUSPICIOUS_STEW_EFFECTS, lv.getStewEffects());
            set.add(lv2);
        }
    ...
    private static void addOminousBottles(ItemGroup.Entries entries, ItemGroup.StackVisibility visibility) {
        for (int i = 0; i <= 4; ++i) {
            ItemStack lv = new ItemStack(Items.OMINOUS_BOTTLE);
            lv.set(DataComponentTypes.OMINOUS_BOTTLE_AMPLIFIER, i);
    ...
    private static void addFireworkRockets(ItemGroup.Entries entries, ItemGroup.StackVisibility visibility) {
        for (byte b : FireworkRocketItem.FLIGHT_VALUES) {
            ...
            lv.set(DataComponentTypes.FIREWORKS, new FireworksComponent(b, List.of()));
            entries.add(lv, visibility);
        }
    }

    private static void addPaintings(ItemGroup.Entries entries, RegistryWrapper.WrapperLookup arg2, RegistryWrapper.Impl<PaintingVariant> arg3, Predicate<RegistryEntry<PaintingVariant>> predicate, ItemGroup.StackVisibility arg42) {
        RegistryOps<NbtElement> lv = arg2.getOps(NbtOps.INSTANCE);
        arg3.streamEntries().filter(predicate).sorted(PAINTING_VARIANT_COMPARATOR).forEach(arg4 -> {
            NbtComponent lv = NbtComponent.DEFAULT.with(lv, PaintingEntity.VARIANT_MAP_CODEC, arg4).getOrThrow().apply(nbt -> nbt.putString("id", "minecraft:painting"));
            ItemStack lv2 = new ItemStack(Items.PAINTING);
            lv2.set(DataComponentTypes.ENTITY_DATA, lv);
            entries.add(lv2, arg42);
        });
    }
