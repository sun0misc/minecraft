...
public class SwampHutGenerator extends ShiftableStructurePiece {
   ...
   public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
      if (this.adjustToAverageHeight(world, chunkBox, 0)) {
         ...
         if (!this.hasWitch) {
            BlockPos lv5 = this.offsetPos(2, 2, 5);
            if (chunkBox.contains(lv5)) {
               this.hasWitch = true;
               WitchEntity lv6 = (WitchEntity)EntityType.WITCH.create(world.toServerWorld());
               if (lv6 != null) {
                  lv6.setPersistent();
                  lv6.refreshPositionAndAngles((double)lv5.getX() + 0.5, (double)lv5.getY(), (double)lv5.getZ() + 0.5, 0.0F, 0.0F);
                  lv6.initialize(world, world.getLocalDifficulty(lv5), SpawnReason.STRUCTURE, (EntityData)null, (NbtCompound)null);
   ...
   private void spawnCat(ServerWorldAccess world, BlockBox box) {
      if (!this.hasCat) {
         BlockPos lv = this.offsetPos(2, 2, 5);
         if (box.contains(lv)) {
            this.hasCat = true;
            CatEntity lv2 = (CatEntity)EntityType.CAT.create(world.toServerWorld());
            if (lv2 != null) {
               lv2.setPersistent();
               lv2.refreshPositionAndAngles((double)lv.getX() + 0.5, (double)lv.getY(), (double)lv.getZ() + 0.5, 0.0F, 0.0F);
               lv2.initialize(world, world.getLocalDifficulty(lv), SpawnReason.STRUCTURE, (EntityData)null, (NbtCompound)null);
