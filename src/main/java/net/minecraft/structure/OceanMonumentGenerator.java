...
public class OceanMonumentGenerator {
   ...
   public static class Base extends Piece {
      ...
      public Base(Random random, int x, int z, Direction orientation) {
         ...
         List list = this.method_14760(random);
         ...
         while(true) {
            while(true) {
               ...
               do {
                  do {
                     if (!var7.hasNext()) {
                        BlockPos lv3 = this.offsetPos(9, 0, 22);
                        Iterator var13 = this.children.iterator();

                        while(var13.hasNext()) {
                           Piece lv4 = (Piece)var13.next();
                           lv4.getBoundingBox().move(lv3);
                        }

                        BlockBox lv5 = BlockBox.create(this.offsetPos(1, 1, 1), this.offsetPos(23, 8, 21));
                        ...
                        this.children.add(new WingRoom(orientation, lv5, k++));
                        this.children.add(new WingRoom(orientation, lv6, k++));
                        this.children.add(new Penthouse(orientation, lv7));
                        return;
                     }

                     lv = (PieceSetting)var7.next();
                  } while(lv.used);
               } while(lv.isAboveLevelThree());

               Iterator var9 = list2.iterator();

               while(var9.hasNext()) {
                  PieceFactory lv2 = (PieceFactory)var9.next();
                  if (lv2.canGenerate(lv)) {
                     this.children.add(lv2.generate(orientation, lv, random));
                     break;
                  }
               }
            }
      ...
      private List method_14760(Random random) {
         ...
         this.coreRoomPieceSetting = lvs[getIndex(random.nextInt(4), 0, 2)];
         ...
         PieceSetting[] var20 = lvs;
         var8 = lvs.length;

         for(var9 = 0; var9 < var8; ++var9) {
            PieceSetting lv5 = var20[var9];
            if (lv5 != null) {
         ...
         label95:
         while(var22.hasNext()) {
            PieceSetting lv6 = (PieceSetting)var22.next();
            int r = 0;
            m = 0;
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         ...
         while(var15.hasNext()) {
            Piece lv = (Piece)var15.next();
            if (lv.getBoundingBox().intersects(chunkBox)) {
   ...
   protected abstract static class Piece extends StructurePiece {
      ...
      protected Piece(StructurePieceType type, int length, Direction orientation, PieceSetting setting, int j, int k, int l) {
         super(type, length, createBox(orientation, setting, j, k, l));
         this.setOrientation(orientation);
      ...
      private static BlockBox createBox(Direction orientation, PieceSetting setting, int i, int j, int k) {
         ...
         int n = l / 5 % 5;
         int o = l / 25;
         BlockBox lv = createBox(0, 0, 0, orientation, i * 8, j * 4, k * 8);
         switch (orientation) {
            case NORTH:
      ...
      protected void fillArea(StructureWorldAccess world, BlockBox box, int x, int y, int z, int width, int height, int depth, BlockState state) {
         for(int o = y; o <= height; ++o) {
            for(int p = x; p <= width; ++p) {
               for(int q = z; q <= depth; ++q) {
                  if (this.getBlockAt(world, p, o, q, box) == WATER) {
      ...
      protected void spawnElderGuardian(StructureWorldAccess world, BlockBox box, int x, int y, int z) {
         ...
         if (box.contains(lv)) {
            ElderGuardianEntity lv2 = (ElderGuardianEntity)EntityType.ELDER_GUARDIAN.create(world.toServerWorld());
            if (lv2 != null) {
               lv2.heal(lv2.getMaxHealth());
               lv2.refreshPositionAndAngles((double)lv.getX() + 0.5, (double)lv.getY(), (double)lv.getZ() + 0.5, 0.0F, 0.0F);
               lv2.initialize(world, world.getLocalDifficulty(lv2.getBlockPos()), SpawnReason.STRUCTURE, (EntityData)null, (NbtCompound)null);
