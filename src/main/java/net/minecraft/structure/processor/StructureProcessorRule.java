...
public class StructureProcessorRule {
    ...
    public static final Codec<StructureProcessorRule> CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)RuleTest.TYPE_CODEC.fieldOf("input_predicate")).forGetter(rule -> rule.inputPredicate), ((MapCodec)RuleTest.TYPE_CODEC.fieldOf("location_predicate")).forGetter(rule -> rule.locationPredicate), PosRuleTest.BASE_CODEC.lenientOptionalFieldOf("position_predicate", AlwaysTruePosRuleTest.INSTANCE).forGetter(rule -> rule.positionPredicate), ((MapCodec)BlockState.CODEC.fieldOf("output_state")).forGetter(rule -> rule.outputState), RuleBlockEntityModifier.TYPE_CODEC.lenientOptionalFieldOf("block_entity_modifier", DEFAULT_BLOCK_ENTITY_MODIFIER).forGetter(rule -> rule.blockEntityModifier)).apply((Applicative<StructureProcessorRule, ?>)instance, StructureProcessorRule::new));
