...
public class DefaultAttributeRegistry {
    ...
    public static void checkMissing() {
        Registries.ENTITY_TYPE.stream().filter(entityType -> entityType.getSpawnGroup() != SpawnGroup.MISC).filter(entityType -> !DefaultAttributeRegistry.hasDefinitionFor(entityType)).map(Registries.ENTITY_TYPE::getId).forEach(id -> Util.error("Entity " + String.valueOf(id) + " has no attributes"));
    }
}
