...
extends ThrownItemEntity {
    ...
    private ParticleEffect getParticleParameters() {
        ...
        return lv.isEmpty() || lv.isOf(this.getDefaultItem()) ? ParticleTypes.ITEM_SNOWBALL : new ItemStackParticleEffect(ParticleTypes.ITEM, lv);
    }
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.PLAY_DEATH_SOUND_OR_ADD_PROJECTILE_HIT_PARTICLES) {
            ...
            for (int i = 0; i < 8; ++i) {
                this.getWorld().addParticle(lv, this.getX(), this.getY(), this.getZ(), 0.0, 0.0, 0.0);
            }
        }
    }
