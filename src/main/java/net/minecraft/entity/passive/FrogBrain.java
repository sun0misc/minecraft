...
public class FrogBrain {
   ...
   protected static void coolDownLongJump(FrogEntity frog, Random random) {
      frog.getBrain().remember(MemoryModuleType.LONG_JUMP_COOLING_DOWN, (Object)longJumpCooldownRange.get(random));
   }
   ...
   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(0, new BreedTask(EntityType.FROG, 1.0F)), Pair.of(1, new TemptTask((frog) -> {
         return 1.25F;
      })), Pair.of(2, UpdateAttackTargetTask.create(FrogBrain::isNotBreeding, (frog) -> {
         return frog.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
      })), Pair.of(3, WalkTowardsLandTask.create(6, 1.0F)), Pair.of(4, new RandomTask(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(StrollTask.create(1.0F), 1), Pair.of(GoTowardsLookTargetTask.create(1.0F, 3), 1), Pair.of(new CroakTask(), 3), Pair.of(TaskTriggerer.predicate(Entity::isOnGround), 2))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_IN_WATER, MemoryModuleState.VALUE_ABSENT)));
   }
   ...
   private static void addLaySpawnActivities(Brain brain) {
      brain.setTaskList(Activity.LAY_SPAWN, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(1, UpdateAttackTargetTask.create(FrogBrain::isNotBreeding, (frog) -> {
         return frog.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
      })), Pair.of(2, WalkTowardsWaterTask.create(8, 1.0F)), Pair.of(3, LayFrogSpawnTask.create(Blocks.FROGSPAWN)), Pair.of(4, new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(1.0F), 2), Pair.of(GoTowardsLookTargetTask.create(1.0F, 3), 1), Pair.of(new CroakTask(), 2), Pair.of(TaskTriggerer.predicate(Entity::isOnGround), 1))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_PREGNANT, MemoryModuleState.VALUE_PRESENT)));
   }

   private static void addLongJumpActivities(Brain brain) {
      brain.setTaskList(Activity.LONG_JUMP, ImmutableList.of(Pair.of(0, new LeapingChargeTask(longJumpCooldownRange, SoundEvents.ENTITY_FROG_STEP)), Pair.of(1, new BiasedLongJumpTask(longJumpCooldownRange, 2, 4, 1.5F, (frog) -> {
   ...
   private static boolean shouldJumpTo(MobEntity frog, BlockPos pos) {
      ...
      if (lv.getFluidState(pos).isEmpty() && lv.getFluidState(lv2).isEmpty() && lv.getFluidState(pos.up()).isEmpty()) {
         ...
         if (!lv3.isIn(BlockTags.FROG_PREFER_JUMP_TO) && !lv4.isIn(BlockTags.FROG_PREFER_JUMP_TO)) {
            PathNodeType lv5 = LandPathNodeMaker.getLandNodeType(lv, pos.mutableCopy());
            PathNodeType lv6 = LandPathNodeMaker.getLandNodeType(lv, lv2.mutableCopy());
            return lv5 != PathNodeType.TRAPDOOR && (!lv3.isAir() || lv6 != PathNodeType.TRAPDOOR) ? LongJumpTask.shouldJumpTo(frog, pos) : true;
         } else {
            return true;
         }
      } else {
