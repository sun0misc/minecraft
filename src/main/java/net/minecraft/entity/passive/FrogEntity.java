...
implements VariantHolder<RegistryEntry<FrogVariant>> {
    ...
    @Override
    public void setVariant(RegistryEntry<FrogVariant> arg) {
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        nbt.putString(VARIANT_KEY, this.getVariant().getKey().orElse(DEFAULT_VARIANT_KEY).getValue().toString());
    }

    @Override
    public void readCustomDataFromNbt(NbtCompound nbt) {
        super.readCustomDataFromNbt(nbt);
        Optional.ofNullable(Identifier.tryParse(nbt.getString(VARIANT_KEY))).map(variant -> RegistryKey.of(RegistryKeys.FROG_VARIANT, variant)).flatMap(Registries.FROG_VARIANT::getEntry).ifPresent(this::setVariant);
    }
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        FrogEntity lv = EntityType.FROG.create(world);
        if (lv != null) {
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        ...
        if (lv.isIn(BiomeTags.SPAWNS_COLD_VARIANT_FROGS)) {
            this.setVariant((RegistryEntry<FrogVariant>)Registries.FROG_VARIANT.entryOf(FrogVariant.COLD));
        } else if (lv.isIn(BiomeTags.SPAWNS_WARM_VARIANT_FROGS)) {
            this.setVariant((RegistryEntry<FrogVariant>)Registries.FROG_VARIANT.entryOf(FrogVariant.WARM));
        } else {
            this.setVariant((RegistryEntry<FrogVariant>)Registries.FROG_VARIANT.entryOf(DEFAULT_VARIANT_KEY));
        }
    ...
    public static DefaultAttributeContainer.Builder createFrogAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 1.0).add(EntityAttributes.GENERIC_MAX_HEALTH, 10.0).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 10.0).add(EntityAttributes.GENERIC_STEP_HEIGHT, 1.0);
    }
