...
public class PandaEntity extends AnimalEntity {
   ...
   private void sneeze() {
      Vec3d lv = this.getVelocity();
      this.world.addParticle(ParticleTypes.SNEEZE, this.getX() - (double)(this.getWidth() + 1.0F) * 0.5 * (double)MathHelper.sin(this.bodyYaw * 0.017453292F), this.getEyeY() - 0.10000000149011612, this.getZ() + (double)(this.getWidth() + 1.0F) * 0.5 * (double)MathHelper.cos(this.bodyYaw * 0.017453292F), lv.x, 0.0, lv.z);
      this.playSound(SoundEvents.ENTITY_PANDA_SNEEZE, 1.0F, 1.0F);
      List list = this.world.getNonSpectatingEntities(PandaEntity.class, this.getBoundingBox().expand(10.0));
      Iterator var3 = list.iterator();

      while(var3.hasNext()) {
         PandaEntity lv2 = (PandaEntity)var3.next();
         if (!lv2.isBaby() && lv2.onGround && !lv2.isTouchingWater() && lv2.isIdle()) {
            lv2.jump();
         }
      }
   ...
   class PickUpFoodGoal extends Goal {
      ...
      public boolean canStart() {
         if (this.startAge <= PandaEntity.this.age && !PandaEntity.this.isBaby() && !PandaEntity.this.isTouchingWater() && PandaEntity.this.isIdle() && PandaEntity.this.getAskForBambooTicks() <= 0) {
            List list = PandaEntity.this.world.getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(6.0, 6.0, 6.0), PandaEntity.IS_FOOD);
      ...
      public void start() {
         List list = PandaEntity.this.world.getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), PandaEntity.IS_FOOD);
