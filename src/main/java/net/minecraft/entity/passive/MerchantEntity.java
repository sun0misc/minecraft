...
Merchant {
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        if (!this.getWorld().isClient && !(lv = this.getOffers()).isEmpty()) {
            nbt.put("Offers", TradeOfferList.CODEC.encodeStart(this.getRegistryManager().getOps(NbtOps.INSTANCE), lv).getOrThrow());
        }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("Offers")) {
            TradeOfferList.CODEC.parse(this.getRegistryManager().getOps(NbtOps.INSTANCE), nbt.get("Offers")).resultOrPartial(Util.addPrefix("Failed to load offers: ", LOGGER::warn)).ifPresent(offers -> {
                this.offers = offers;
            });
        }
