...
Targeter {
    ...
    protected void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        if (this.attack != null) {
            Interaction.CODEC.encodeStart(NbtOps.INSTANCE, this.attack).ifSuccess(arg2 -> nbt.put(ATTACK_KEY, (NbtElement)arg2));
        }
        if (this.interaction != null) {
            Interaction.CODEC.encodeStart(NbtOps.INSTANCE, this.interaction).ifSuccess(arg2 -> nbt.put(INTERACTION_KEY, (NbtElement)arg2));
        }
    ...
    public boolean handleAttack(Entity attacker) {
        if (attacker instanceof PlayerEntity) {
            ...
            this.attack = new Interaction(lv.getUuid(), this.getWorld().getTime());
