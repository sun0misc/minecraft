...
extends Goal {
    ...
    private void moveToAir() {
        ...
        for (BlockPos lv2 : iterable) {
            if (!this.isAirPos(this.mob.getWorld(), lv2)) continue;
            lv = lv2;
            break;
        }
        if (lv == null) {
    ...
    private boolean isAirPos(WorldView world, BlockPos pos) {
        ...
        return (world.getFluidState(pos).isEmpty() || lv.isOf(Blocks.BUBBLE_COLUMN)) && lv.canPathfindThrough(NavigationType.LAND);
    }
}
