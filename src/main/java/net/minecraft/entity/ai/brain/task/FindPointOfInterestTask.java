...
public class FindPointOfInterestTask {
    ...
    public static Task<PathAwareEntity> create(Predicate<RegistryEntry<PointOfInterestType>> poiPredicate, MemoryModuleType<GlobalPos> poiPosModule, MemoryModuleType<GlobalPos> potentialPoiPosModule, boolean onlyRunIfChild, Optional<Byte> entityStatus) {
        ...
        SingleTickTask<PathAwareEntity> lv = TaskTriggerer.task(arg2 -> arg2.group(arg2.queryMemoryAbsent(potentialPoiPosModule)).apply(arg2, queryResult -> (world, entity, time) -> {
            ...
            Set<Pair<RegistryEntry<PointOfInterestType>, BlockPos>> set = lv.getSortedTypesAndPositions(poiPredicate, predicate2, entity.getBlockPos(), 48, PointOfInterestStorage.OccupationStatus.HAS_SPACE).limit(5L).collect(Collectors.toSet());
            ...
            if (lv2 != null && lv2.reachesTarget()) {
                ...
                for (Pair<RegistryEntry<PointOfInterestType>, BlockPos> pair : set) {
                    long2ObjectMap.computeIfAbsent(pair.getSecond().asLong(), m -> new RetryMarker(arg.random, time));
                }
            }
            return true;
        }));
