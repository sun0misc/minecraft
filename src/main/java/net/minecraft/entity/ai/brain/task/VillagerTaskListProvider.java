...
public class VillagerTaskListProvider {
   ...
   public static ImmutableList createWorkTasks(VillagerProfession profession, float speed) {
      Object lv;
      if (profession == VillagerProfession.FARMER) {
         lv = new FarmerWorkTask();
      } else {
         lv = new VillagerWorkTask();
      }

      return ImmutableList.of(createBusyFollowTask(), Pair.of(5, new RandomTask(ImmutableList.of(Pair.of(lv, 7), Pair.of(GoToIfNearbyTask.create(MemoryModuleType.JOB_SITE, 0.4F, 4), 2), Pair.of(GoToNearbyPositionTask.create(MemoryModuleType.JOB_SITE, 0.4F, 1, 10), 5), Pair.of(GoToSecondaryPositionTask.create(MemoryModuleType.SECONDARY_JOB_SITE, speed, 1, 6, MemoryModuleType.JOB_SITE), 5), Pair.of(new FarmerVillagerTask(), profession == VillagerProfession.FARMER ? 2 : 5), Pair.of(new BoneMealTask(), profession == VillagerProfession.FARMER ? 4 : 7)))), Pair.of(10, new HoldTradeOffersTask(400, 1600)), Pair.of(10, FindInteractionTargetTask.create(EntityType.PLAYER, 4)), Pair.of(2, VillagerWalkTowardsTask.create(MemoryModuleType.JOB_SITE, speed, 9, 100, 1200)), Pair.of(3, new GiveGiftsToHeroTask(100)), Pair.of(99, ScheduleActivityTask.create()));
   }

   public static ImmutableList createPlayTasks(float speed) {
      return ImmutableList.of(Pair.of(0, new WanderAroundTask(80, 120)), createFreeFollowTask(), Pair.of(5, PlayWithVillagerBabiesTask.create()), Pair.of(5, new RandomTask(ImmutableMap.of(MemoryModuleType.VISIBLE_VILLAGER_BABIES, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(FindEntityTask.create(EntityType.VILLAGER, 8, MemoryModuleType.INTERACTION_TARGET, speed, 2), 2), Pair.of(FindEntityTask.create(EntityType.CAT, 8, MemoryModuleType.INTERACTION_TARGET, speed, 2), 1), Pair.of(FindWalkTargetTask.create(speed), 1), Pair.of(GoTowardsLookTargetTask.create(speed, 2), 1), Pair.of(new JumpInBedTask(speed), 2), Pair.of(new WaitTask(20, 40), 2)))), Pair.of(99, ScheduleActivityTask.create()));
   }

   public static ImmutableList createRestTasks(VillagerProfession profession, float speed) {
      return ImmutableList.of(Pair.of(2, VillagerWalkTowardsTask.create(MemoryModuleType.HOME, speed, 1, 150, 1200)), Pair.of(3, ForgetCompletedPointOfInterestTask.create((poiType) -> {
         return poiType.matchesKey(PointOfInterestTypes.HOME);
      }, MemoryModuleType.HOME)), Pair.of(3, new SleepTask()), Pair.of(5, new RandomTask(ImmutableMap.of(MemoryModuleType.HOME, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(WalkHomeTask.create(speed), 1), Pair.of(WanderIndoorsTask.create(speed), 4), Pair.of(GoToPointOfInterestTask.create(speed, 4), 2), Pair.of(new WaitTask(20, 40), 2)))), createBusyFollowTask(), Pair.of(99, ScheduleActivityTask.create()));
   }

   public static ImmutableList createMeetTasks(VillagerProfession profession, float speed) {
   ...
   public static ImmutableList createIdleTasks(VillagerProfession profession, float speed) {
   ...
   public static ImmutableList createPanicTasks(VillagerProfession profession, float speed) {
      float g = speed * 1.5F;
      return ImmutableList.of(Pair.of(0, StopPanickingTask.create()), Pair.of(1, GoToRememberedPositionTask.createEntityBased(MemoryModuleType.NEAREST_HOSTILE, g, 6, false)), Pair.of(1, GoToRememberedPositionTask.createEntityBased(MemoryModuleType.HURT_BY_ENTITY, g, 6, false)), Pair.of(3, FindWalkTargetTask.create(g, 2, 2)), createBusyFollowTask());
   }

   public static ImmutableList createPreRaidTasks(VillagerProfession profession, float speed) {
      return ImmutableList.of(Pair.of(0, RingBellTask.create()), Pair.of(0, Tasks.pickRandomly(ImmutableList.of(Pair.of(VillagerWalkTowardsTask.create(MemoryModuleType.MEETING_POINT, speed * 1.5F, 2, 150, 200), 6), Pair.of(FindWalkTargetTask.create(speed * 1.5F), 2)))), createBusyFollowTask(), Pair.of(99, EndRaidTask.create()));
   }

   public static ImmutableList createRaidTasks(VillagerProfession profession, float speed) {
      return ImmutableList.of(Pair.of(0, TaskTriggerer.runIf((TaskRunnable)TaskTriggerer.predicate(VillagerTaskListProvider::wonRaid), (TaskRunnable)Tasks.pickRandomly(ImmutableList.of(Pair.of(SeekSkyTask.create(speed), 5), Pair.of(FindWalkTargetTask.create(speed * 1.1F), 2))))), Pair.of(0, new CelebrateRaidWinTask(600, 600)), Pair.of(2, TaskTriggerer.runIf((TaskRunnable)TaskTriggerer.predicate(VillagerTaskListProvider::hasActiveRaid), (TaskRunnable)HideInHomeTask.create(24, speed * 1.4F, 1))), createBusyFollowTask(), Pair.of(99, EndRaidTask.create()));
   }

   public static ImmutableList createHideTasks(VillagerProfession profession, float speed) {
      int i = true;
