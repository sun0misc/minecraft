...
RangedAttackMob {
    ...
    public void tickMovement() {
        ...
        if (!this.getWorld().isClient && this.getTrackedEntityId(0) > 0 && (lv2 = this.getWorld().getEntityById(this.getTrackedEntityId(0))) != null) {
            double d = lv.y;
        ...
        for (i = 0; i < 2; ++i) {
            ...
            Entity lv5 = null;
            if (j > 0) {
                lv5 = this.getWorld().getEntityById(j);
            }
            if (lv5 != null) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            this.getWorld().addParticle(EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.7f, 0.7f, 0.5f), p + this.random.nextGaussian() * (double)s, q + this.random.nextGaussian() * (double)s, r + this.random.nextGaussian() * (double)s, 0.0, 0.0, 0.0);
        }
        if (this.getInvulnerableTimer() > 0) {
            float t = 3.3f * this.getScale();
            for (int u = 0; u < 3; ++u) {
                this.getWorld().addParticle(EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.7f, 0.7f, 0.9f), this.getX() + this.random.nextGaussian(), this.getY() + (double)(this.random.nextFloat() * t), this.getZ() + this.random.nextGaussian(), 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    protected void mobTick() {
        ...
        for (int i = 1; i < 3; ++i) {
            ...
            if ((j = this.getTrackedEntityId(i)) > 0) {
                LivingEntity lv = (LivingEntity)this.getWorld().getEntityById(j);
        ...
        if (this.blockBreakingCooldown > 0) {
            ...
            if (this.blockBreakingCooldown == 0 && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                ...
                for (BlockPos lv3 : BlockPos.iterate(this.getBlockX() - j, this.getBlockY(), this.getBlockZ() - j, this.getBlockX() + j, this.getBlockY() + k, this.getBlockZ() + j)) {
                    ...
                    bl = this.getWorld().breakBlock(lv3, true, this) || bl;
                }
                if (bl) {
    ...
    private void shootSkullAt(int headIndex, double targetX, double targetY, double targetZ, boolean charged) {
        ...
        WitherSkullEntity lv2 = new WitherSkullEntity(this.getWorld(), this, lv.normalize());
        lv2.setOwner(this);
        if (charged) {
            lv2.setCharged(true);
        }
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (this.shouldRenderOverlay() && ((lv = source.getSource()) instanceof PersistentProjectileEntity || lv instanceof WindChargeEntity)) {
            return false;
        }
