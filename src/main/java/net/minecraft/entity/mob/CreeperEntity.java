...
implements SkinOverlayOwner {
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (lv.isIn(ItemTags.CREEPER_IGNITERS)) {
            ...
            this.getWorld().playSound(player, this.getX(), this.getY(), this.getZ(), lv2, this.getSoundCategory(), 1.0f, this.random.nextFloat() * 0.4f + 0.8f);
            if (!this.getWorld().isClient) {
                this.ignite();
                if (!lv.isDamageable()) {
                    lv.decrement(1);
                } else {
                    lv.damage(1, player, CreeperEntity.getSlotForHand(hand));
                }
            }
