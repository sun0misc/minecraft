...
public class WardenEntity extends HostileEntity implements VibrationListener.Callback {
   ...
   private final EntityGameEventHandler gameEventHandler = new EntityGameEventHandler(new VibrationListener(new EntityPositionSource(this, this.getStandingEyeHeight()), this));
   ...
   public void tick() {
      ...
      if (var2 instanceof ServerWorld lv) {
         ((VibrationListener)this.gameEventHandler.getListener()).tick(lv);
      ...
      if ((float)animationState.getTimeRunning() < 4500.0F) {
         ...
         if (lv2.getRenderType() != BlockRenderType.INVISIBLE) {
            for(int i = 0; i < 30; ++i) {
               ...
               this.world.addParticle(new BlockStateParticleEffect(ParticleTypes.BLOCK, lv2), d, e, f, 0.0, 0.0, 0.0);
            }
         }
      }
   ...
   public boolean isValidTarget(@Nullable Entity entity) {
      ...
      if (entity instanceof LivingEntity lv) {
         if (this.world == entity.world && EntityPredicates.EXCEPT_CREATIVE_OR_SPECTATOR.test(entity) && !this.isTeammate(entity) && lv.getType() != EntityType.ARMOR_STAND && lv.getType() != EntityType.WARDEN && !lv.isInvulnerable() && !lv.isDead() && this.world.getWorldBorder().contains(lv.getBoundingBox())) {
            var10000 = true;
            return var10000;
         }
      }
   ...
   public void writeCustomDataToNbt(NbtCompound nbt) {
      ...
      var10000 = VibrationListener.createCodec(this).encodeStart(NbtOps.INSTANCE, (VibrationListener)this.gameEventHandler.getListener());
      var10001 = LOGGER;
      Objects.requireNonNull(var10001);
   ...
   public void readCustomDataFromNbt(NbtCompound nbt) {
      ...
      if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
         var10000 = VibrationListener.createCodec(this).parse(new Dynamic(NbtOps.INSTANCE, nbt.getCompound("listener")));
         var10001 = LOGGER;
         Objects.requireNonNull(var10001);
         var10000.resultOrPartial(var10001::error).ifPresent((arg) -> {
            this.gameEventHandler.setListener(arg, this.world);
         });
      }
   ...
   public boolean accepts(ServerWorld world, GameEventListener listener, BlockPos pos, GameEvent event, GameEvent.Emitter emitter) {
      if (!this.isAiDisabled() && !this.isDead() && !this.getBrain().hasMemoryModule(MemoryModuleType.VIBRATION_COOLDOWN) && !this.isDiggingOrEmerging() && world.getWorldBorder().contains(pos)) {
         ...
         if (var7 instanceof LivingEntity) {
            LivingEntity lv = (LivingEntity)var7;
            if (!this.isValidTarget(lv)) {
               var10000 = false;
               return var10000;
            }
         }
   ...
   public void accept(ServerWorld world, GameEventListener listener, BlockPos pos, GameEvent event, @Nullable Entity entity, @Nullable Entity sourceEntity, float distance) {
      if (!this.isDead()) {
         ...
         if (!this.getAngriness().isAngry()) {
            Optional optional = this.angerManager.getPrimeSuspect();
            if (sourceEntity != null || optional.isEmpty() || optional.get() == entity) {
               WardenBrain.lookAtDisturbance(this, lv);
            }
         }
