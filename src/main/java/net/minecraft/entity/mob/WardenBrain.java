...
public class WardenBrain {
   ...
   protected static Brain create(WardenEntity warden, Dynamic dynamic) {
      Brain.Profile lv = Brain.createProfile(MEMORY_MODULES, SENSORS);
      Brain lv2 = lv.deserialize(dynamic);
      addCoreActivities(lv2);
      addEmergeActivities(lv2);
      addDigActivities(lv2);
      addIdleActivities(lv2);
      addRoarActivities(lv2);
      addFightActivities(warden, lv2);
      addInvestigateActivities(lv2);
      addSniffActivities(lv2);
      lv2.setCoreActivities(ImmutableSet.of(Activity.CORE));
      lv2.setDefaultActivity(Activity.IDLE);
      lv2.resetPossibleActivities();
      return lv2;
   }
   ...
   private static void addFightActivities(WardenEntity warden, Brain brain) {
      brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(RESET_DIG_COOLDOWN_TASK, ForgetAttackTargetTask.create((entity) -> {
         return !warden.getAngriness().isAngry() || !warden.isValidTarget(entity);
      }, WardenBrain::removeDeadSuspect, false), LookAtMobTask.create((entity) -> {
         return isTargeting(warden, entity);
      }, (float)warden.getAttributeValue(EntityAttributes.GENERIC_FOLLOW_RANGE)), RangedApproachTask.create(1.2F), new SonicBoomTask(), MeleeAttackTask.create(18)), MemoryModuleType.ATTACK_TARGET);
   }
