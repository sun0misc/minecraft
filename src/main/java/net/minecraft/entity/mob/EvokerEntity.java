...
extends SpellcastingIllagerEntity {
    ...
    public boolean isTeammate(Entity other) {
        ...
        if (other instanceof VexEntity) {
            VexEntity lv = (VexEntity)other;
            return this.isTeammate(lv.getOwner());
        }
        return false;
    }
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        protected void castSpell() {
            ServerWorld lv = (ServerWorld)EvokerEntity.this.getWorld();
            Team lv2 = EvokerEntity.this.getScoreboardTeam();
            for (int i = 0; i < 3; ++i) {
                BlockPos lv3 = EvokerEntity.this.getBlockPos().add(-2 + EvokerEntity.this.random.nextInt(5), 1, -2 + EvokerEntity.this.random.nextInt(5));
                VexEntity lv4 = EntityType.VEX.create(EvokerEntity.this.getWorld());
                if (lv4 == null) continue;
                lv4.refreshPositionAndAngles(lv3, 0.0f, 0.0f);
                lv4.initialize(lv, EvokerEntity.this.getWorld().getLocalDifficulty(lv3), SpawnReason.MOB_SUMMONED, null);
                lv4.setOwner(EvokerEntity.this);
                lv4.setBounds(lv3);
                lv4.setLifeTicks(20 * (30 + EvokerEntity.this.random.nextInt(90)));
                if (lv2 != null) {
                    lv.getScoreboard().addScoreHolderToTeam(lv4.getNameForScoreboard(), lv2);
                }
                lv.spawnEntityAndPassengers(lv4);
                lv.emitGameEvent(GameEvent.ENTITY_PLACE, lv3, GameEvent.Emitter.of(EvokerEntity.this));
            }
        }
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        private void conjureFangs(double x, double z, double maxY, double y, float yaw, int warmup) {
            ...
            do {
                ...
                BlockState lv3 = EvokerEntity.this.getWorld().getBlockState(lv2);
                if (!lv3.isSideSolidFullSquare(EvokerEntity.this.getWorld(), lv2, Direction.UP)) continue;
                if (!EvokerEntity.this.getWorld().isAir(lv) && !(lv5 = (lv4 = EvokerEntity.this.getWorld().getBlockState(lv)).getCollisionShape(EvokerEntity.this.getWorld(), lv)).isEmpty()) {
            ...
            if (bl) {
                EvokerEntity.this.getWorld().spawnEntity(new EvokerFangsEntity(EvokerEntity.this.getWorld(), x, (double)lv.getY() + j, z, yaw, warmup, EvokerEntity.this));
                EvokerEntity.this.getWorld().emitGameEvent(GameEvent.ENTITY_PLACE, new Vec3d(x, (double)lv.getY() + j, z), GameEvent.Emitter.of(EvokerEntity.this));
            }
        }
