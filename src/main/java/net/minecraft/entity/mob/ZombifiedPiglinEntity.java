...
implements Angerable {
    ...
    private static final EntityAttributeModifier ATTACKING_SPEED_BOOST = new EntityAttributeModifier(ATTACKING_SPEED_BOOST_ID, 0.05, EntityAttributeModifier.Operation.ADD_VALUE);
    ...
    protected void mobTick() {
        ...
        if (this.hasAngerTime()) {
            if (!this.isBaby() && !lv.hasModifier(ATTACKING_SPEED_BOOST_ID)) {
                lv.addTemporaryModifier(ATTACKING_SPEED_BOOST);
            }
            this.tickAngrySound();
        } else if (lv.hasModifier(ATTACKING_SPEED_BOOST_ID)) {
