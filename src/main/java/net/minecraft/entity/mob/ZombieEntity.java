...
extends HostileEntity {
    ...
    private static final EntityAttributeModifier BABY_SPEED_BONUS = new EntityAttributeModifier(BABY_SPEED_ID, 0.5, EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE);
    private static final Identifier field_52018 = Identifier.method_60656("reinforcement_caller_charge");
    private static final EntityAttributeModifier field_52016 = new EntityAttributeModifier(Identifier.method_60656("reinforcement_callee_charge"), -0.05f, EntityAttributeModifier.Operation.ADD_VALUE);
    private static final Identifier field_52017 = Identifier.method_60656("leader_zombie_bonus");
    private static final Identifier field_52193 = Identifier.method_60656("zombie_random_spawn_bonus");
    ...
    public void setBaby(boolean baby) {
        ...
        if (this.getWorld() != null && !this.getWorld().isClient) {
            ...
            lv.removeModifier(BABY_SPEED_ID);
            if (baby) {
    ...
    protected void convertTo(EntityType<? extends ZombieEntity> entityType) {
        ...
        if (lv != null) {
            lv.applyAttributeModifiers(lv.getWorld().getLocalDifficulty(lv.getBlockPos()).getClampedLocalDifficulty());
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (lv2 != null && this.getWorld().getDifficulty() == Difficulty.HARD && (double)this.random.nextFloat() < this.getAttributeValue(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS) && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_SPAWNING)) {
            int i = MathHelper.floor(this.getX());
            int j = MathHelper.floor(this.getY());
            int k = MathHelper.floor(this.getZ());
            ZombieEntity lv3 = new ZombieEntity(this.getWorld());
            for (int l = 0; l < 50; ++l) {
                ...
                if (!SpawnRestriction.isSpawnPosAllowed(lv5, this.getWorld(), lv4) || !SpawnRestriction.canSpawn(lv5, lv, SpawnReason.REINFORCEMENT, lv4, this.getWorld().random)) continue;
                lv3.setPosition(m, n, o);
                if (this.getWorld().isPlayerInRange(m, n, o, 7.0) || !this.getWorld().doesNotIntersectEntities(lv3) || !this.getWorld().isSpaceEmpty(lv3) || this.getWorld().containsFluid(lv3.getBoundingBox())) continue;
                lv3.setTarget(lv2);
                lv3.initialize(lv, this.getWorld().getLocalDifficulty(lv3.getBlockPos()), SpawnReason.REINFORCEMENT, null);
                lv.spawnEntityAndPassengers(lv3);
                EntityAttributeInstance lv6 = this.getAttributeInstance(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS);
                EntityAttributeModifier lv7 = lv6.getModifier(field_52018);
                double d = lv7 != null ? lv7.value() : 0.0;
                lv6.addPersistentModifier(new EntityAttributeModifier(field_52018, d - 0.05, EntityAttributeModifier.Operation.ADD_VALUE));
                lv3.getAttributeInstance(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS).addPersistentModifier(field_52016);
                break;
            }
        }
        return true;
    ...
    public boolean onKilledOther(ServerWorld world, LivingEntity other) {
        ...
        if ((world.getDifficulty() == Difficulty.NORMAL || world.getDifficulty() == Difficulty.HARD) && other instanceof VillagerEntity) {
            ...
            if (lv2 != null) {
                lv2.initialize(world, world.getLocalDifficulty(lv2.getBlockPos()), SpawnReason.CONVERSION, new ZombieData(false, true));
                lv2.setVillagerData(lv.getVillagerData());
                lv2.setGossipData(lv.getGossip().serialize(NbtOps.INSTANCE));
                lv2.setOfferData(lv.getOffers().copy());
        ...
        if (entityData instanceof ZombieData) {
            ...
            if (lv2.baby) {
                ...
                if (lv2.tryChickenJockey) {
                    ...
                    } else if ((double)lv.nextFloat() < 0.05 && (lv4 = EntityType.CHICKEN.create(this.getWorld())) != null) {
    ...
    protected void applyAttributeModifiers(float chanceMultiplier) {
        this.initAttributes();
        this.getAttributeInstance(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE).addPersistentModifier(new EntityAttributeModifier(field_51997, this.random.nextDouble() * (double)0.05f, EntityAttributeModifier.Operation.ADD_VALUE));
        double d = this.random.nextDouble() * 1.5 * (double)chanceMultiplier;
        if (d > 1.0) {
            this.getAttributeInstance(EntityAttributes.GENERIC_FOLLOW_RANGE).addPersistentModifier(new EntityAttributeModifier(field_52193, d, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));
        }
        if (this.random.nextFloat() < chanceMultiplier * 0.05f) {
            this.getAttributeInstance(EntityAttributes.ZOMBIE_SPAWN_REINFORCEMENTS).addPersistentModifier(new EntityAttributeModifier(field_52017, this.random.nextDouble() * 0.25 + 0.5, EntityAttributeModifier.Operation.ADD_VALUE));
            this.getAttributeInstance(EntityAttributes.GENERIC_MAX_HEALTH).addPersistentModifier(new EntityAttributeModifier(field_52017, this.random.nextDouble() * 3.0 + 1.0, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));
