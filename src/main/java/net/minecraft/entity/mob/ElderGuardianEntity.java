...
extends GuardianEntity {
    ...
    protected void mobTick() {
        ...
        if ((this.age + this.getId()) % 1200 == 0) {
            ...
            List<ServerPlayerEntity> list = StatusEffectUtil.addEffectToPlayersWithinDistance((ServerWorld)this.getWorld(), this, this.getPos(), 50.0, lv, 1200);
            list.forEach(arg -> arg.networkHandler.sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.ELDER_GUARDIAN_EFFECT, this.isSilent() ? GameStateChangeS2CPacket.field_33328 : 1.0f)));
        }
