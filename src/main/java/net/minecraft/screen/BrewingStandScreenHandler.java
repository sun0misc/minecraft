   ...
   public ItemStack quickMove(PlayerEntity player, int slot) {
      ...
      if (lv2 != null && lv2.hasStack()) {
         ...
         if ((slot < 0 || slot > 2) && slot != 3 && slot != 4) {
            if (BrewingStandScreenHandler.FuelSlot.matches(lv)) {
               if (this.insertItem(lv3, 4, 5, false) || this.ingredientSlot.canInsert(lv3) && !this.insertItem(lv3, 3, 4, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (this.ingredientSlot.canInsert(lv3)) {
               if (!this.insertItem(lv3, 3, 4, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (BrewingStandScreenHandler.PotionSlot.matches(lv) && lv.getCount() == 1) {
               if (!this.insertItem(lv3, 0, 3, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (slot >= 5 && slot < 32) {
               if (!this.insertItem(lv3, 32, 41, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (slot >= 32 && slot < 41) {
               if (!this.insertItem(lv3, 5, 32, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (!this.insertItem(lv3, 5, 41, false)) {
               return ItemStack.EMPTY;
            }
         } else {
