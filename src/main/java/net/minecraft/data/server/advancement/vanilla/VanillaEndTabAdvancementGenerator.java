...
implements AdvancementTabGenerator {
    ...
    public void accept(RegistryWrapper.WrapperLookup lookup, Consumer<AdvancementEntry> exporter) {
        AdvancementEntry lv = Advancement.Builder.create().display(Blocks.END_STONE, (Text)Text.translatable("advancements.end.root.title"), (Text)Text.translatable("advancements.end.root.description"), Identifier.method_60656("textures/gui/advancements/backgrounds/end.png"), AdvancementFrame.TASK, false, false, false).criterion("entered_end", ChangedDimensionCriterion.Conditions.to(World.END)).build(exporter, "end/root");
        AdvancementEntry lv2 = Advancement.Builder.create().parent(lv).display(Blocks.DRAGON_HEAD, (Text)Text.translatable("advancements.end.kill_dragon.title"), (Text)Text.translatable("advancements.end.kill_dragon.description"), null, AdvancementFrame.TASK, true, true, false).criterion("killed_dragon", OnKilledCriterion.Conditions.createPlayerKilledEntity(EntityPredicate.Builder.create().type(EntityType.ENDER_DRAGON))).build(exporter, "end/kill_dragon");
        AdvancementEntry lv3 = Advancement.Builder.create().parent(lv2).display(Items.ENDER_PEARL, (Text)Text.translatable("advancements.end.enter_end_gateway.title"), (Text)Text.translatable("advancements.end.enter_end_gateway.description"), null, AdvancementFrame.TASK, true, true, false).criterion("entered_end_gateway", EnterBlockCriterion.Conditions.block(Blocks.END_GATEWAY)).build(exporter, "end/enter_end_gateway");
        ...
        AdvancementEntry lv4 = Advancement.Builder.create().parent(lv3).display(Blocks.PURPUR_BLOCK, (Text)Text.translatable("advancements.end.find_end_city.title"), (Text)Text.translatable("advancements.end.find_end_city.description"), null, AdvancementFrame.TASK, true, true, false).criterion("in_city", TickCriterion.Conditions.createLocation(LocationPredicate.Builder.createStructure(lookup.getWrapperOrThrow(RegistryKeys.STRUCTURE).getOrThrow(StructureKeys.END_CITY)))).build(exporter, "end/find_end_city");
