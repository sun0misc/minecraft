...
extends TagProvider<Enchantment> {
    ...
    protected void createTooltipOrderTag(RegistryWrapper.WrapperLookup registryLookup, RegistryKey<Enchantment> ... enchantments) {
        ...
        List list = registryLookup.getWrapperOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().filter(entry -> !set.contains(entry.getKey().get())).map(RegistryEntry::getIdAsString).collect(Collectors.toList());
        if (!list.isEmpty()) {
