...
public class TextureMap {
   ...
   public Identifier getTexture(TextureKey key) {
      for(TextureKey lv = key; lv != null; lv = lv.getParent()) {
         Identifier lv2 = (Identifier)this.entries.get(lv);
         if (lv2 != null) {
            return lv2;
         }
      }
   ...
   public static TextureMap wallSideTopBottom(Block block) {
      Identifier lv = getId(block);
      return (new TextureMap()).put(TextureKey.WALL, lv).put(TextureKey.SIDE, lv).put(TextureKey.TOP, getSubId(block, "_top")).put(TextureKey.BOTTOM, getSubId(block, "_bottom"));
   }

   public static TextureMap wallSideEnd(Block block) {
      Identifier lv = getId(block);
      return (new TextureMap()).put(TextureKey.TEXTURE, lv).put(TextureKey.WALL, lv).put(TextureKey.SIDE, lv).put(TextureKey.END, getSubId(block, "_top"));
   }
