...
public class ItemModelGenerator {
   ...
   private static final List TRIM_MATERIALS;
   private final BiConsumer writer;

   public ItemModelGenerator(BiConsumer writer) {
      this.writer = writer;
   }
   ...
   private JsonObject createArmorJson(Identifier id, Map textures, ArmorMaterial armorMaterial) {
      JsonObject jsonObject = Models.GENERATED_TWO_LAYERS.createJson(id, textures);
      JsonArray jsonArray = new JsonArray();
      Iterator var6 = TRIM_MATERIALS.iterator();

      while(var6.hasNext()) {
         TrimMaterial lv = (TrimMaterial)var6.next();
   ...
   private void registerArmor(ArmorItem armor) {
      ...
      Identifier lv2 = TextureMap.getId((Item)armor);
      Identifier lv3 = TextureMap.getSubId((Item)armor, "_overlay");
      if (armor.getMaterial() == ArmorMaterials.LEATHER) {
         Models.GENERATED_TWO_LAYERS.upload(lv, TextureMap.layered(lv2, lv3), this.writer, (id, textures) -> {
            return this.createArmorJson(id, textures, armor.getMaterial());
         });
      } else {
         Models.GENERATED.upload(lv, TextureMap.layer0(lv2), this.writer, (id, textures) -> {
            return this.createArmorJson(id, textures, armor.getMaterial());
         });
      }

      Iterator var5 = TRIM_MATERIALS.iterator();

      while(var5.hasNext()) {
         TrimMaterial lv4 = (TrimMaterial)var5.next();
         String string = lv4.getAppliedName(armor.getMaterial());
         Identifier lv5 = this.suffixTrim(lv, string);
         String var10000 = armor.getType().getName();
         String string2 = var10000 + "_trim_" + string;
         Identifier lv6 = (new Identifier(string2)).withPrefixedPath("trims/items/");
         if (armor.getMaterial() == ArmorMaterials.LEATHER) {
   ...
   public void register() {
      ...
      Iterator var1 = Registries.ITEM.iterator();

      while(var1.hasNext()) {
         Item lv = (Item)var1.next();
   ...
   static record TrimMaterial(String name, float itemModelIndex, Map overrideArmorMaterials) {
      TrimMaterial(String string, float f, Map map) {
         this.name = string;
