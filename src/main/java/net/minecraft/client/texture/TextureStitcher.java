...
public class TextureStitcher<T extends Stitchable> {
    ...
    public void add(T info) {
        Holder<T> lv = new Holder<T>(info, this.mipLevel);
        this.holders.add(lv);
    }
    ...
    private boolean growAndFit(Holder<T> holder) {
        ...
        if (bl5) {
            ...
            lv = new Slot<T>(0, this.height, this.width, l - this.height);
            this.height = l;
        }
        lv.fit(holder);
        this.slots.add(lv);
