   ...
   public GlyphRenderer getGlyphRenderer(RenderableGlyph glyph) {
      if (glyph.hasColor() != this.hasColor) {
         ...
         if (lv != null) {
            ...
            float g = 256.0F;
            float h = 0.01F;
            return new GlyphRenderer(this.textLayer, this.seeThroughTextLayer, this.polygonOffsetTextLayer, ((float)lv.x + 0.01F) / 256.0F, ((float)lv.x - 0.01F + (float)glyph.getWidth()) / 256.0F, ((float)lv.y + 0.01F) / 256.0F, ((float)lv.y - 0.01F + (float)glyph.getHeight()) / 256.0F, glyph.getXMin(), glyph.getXMax(), glyph.getYMin(), glyph.getYMax());
         } else {
            return null;
         }
      }
