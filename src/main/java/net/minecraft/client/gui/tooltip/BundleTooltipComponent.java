...
public class BundleTooltipComponent implements TooltipComponent {
   ...
   public void drawItems(TextRenderer textRenderer, int x, int y, MatrixStack matrices, ItemRenderer itemRenderer) {
      ...
      for(int n = 0; n < l; ++n) {
         for(int o = 0; o < k; ++o) {
            int p = x + o * 18 + 1;
            int q = y + n * 20 + 1;
            this.drawSlot(p, q, m++, bl, textRenderer, matrices, itemRenderer);
         }
      }

      this.drawOutline(x, y, k, l, matrices);
   }
   ...
   private void drawSlot(int x, int y, int index, boolean shouldBlock, TextRenderer textRenderer, MatrixStack matrices, ItemRenderer itemRenderer) {
      if (index >= this.inventory.size()) {
         ...
         itemRenderer.renderInGuiWithOverrides(matrices, lv, x + 1, y + 1, index);
         itemRenderer.renderGuiItemOverlay(matrices, textRenderer, lv, x + 1, y + 1);
         if (index == 0) {
   ...
   private void drawOutline(int x, int y, int columns, int rows, MatrixStack matrices) {
      ...
      int m;
      for(m = 0; m < columns; ++m) {
         this.draw(matrices, x + 1 + m * 18, y, BundleTooltipComponent.Sprite.BORDER_HORIZONTAL_TOP);
         this.draw(matrices, x + 1 + m * 18, y + rows * 20, BundleTooltipComponent.Sprite.BORDER_HORIZONTAL_BOTTOM);
      }

      for(m = 0; m < rows; ++m) {
         this.draw(matrices, x, y + m * 20 + 1, BundleTooltipComponent.Sprite.BORDER_VERTICAL);
         this.draw(matrices, x + columns * 18 + 1, y + m * 20 + 1, BundleTooltipComponent.Sprite.BORDER_VERTICAL);
      }
