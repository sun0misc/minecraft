...
public class StructureBlockScreen extends Screen {
   ...
   private CyclingButtonWidget buttonEntities;
   private CyclingButtonWidget buttonMirror;
   private CyclingButtonWidget buttonShowAir;
   private CyclingButtonWidget buttonShowBoundingBox;
   ...
   protected void init() {
      ...
      this.buttonEntities = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(!this.structureBlock.shouldIgnoreEntities()).omitKeyText().build(this.width / 2 + 4 + 100, 160, 50, 20, INCLUDE_ENTITIES_TEXT, (button, includeEntities) -> {
         this.structureBlock.setIgnoreEntities(!includeEntities);
      }));
      this.buttonMirror = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.builder(BlockMirror::getName).values((Object[])BlockMirror.values()).omitKeyText().initially(this.mirror).build(this.width / 2 - 20, 185, 40, 20, Text.literal("MIRROR"), (button, mirror) -> {
         this.structureBlock.setMirror(mirror);
      }));
      this.buttonShowAir = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowAir()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_AIR_TEXT, (button, showAir) -> {
         this.structureBlock.setShowAir(showAir);
      }));
      this.buttonShowBoundingBox = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowBoundingBox()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_BOUNDING_BOX_TEXT, (button, showBoundingBox) -> {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      if (lv == StructureBlockMode.LOAD || lv == StructureBlockMode.SAVE) {
         ...
         drawTextWithShadow(matrices, this.textRenderer, INCLUDE_ENTITIES_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth((StringVisitable)INCLUDE_ENTITIES_TEXT), 150, 10526880);
      }
      ...
      if (lv == StructureBlockMode.SAVE) {
         ...
         drawTextWithShadow(matrices, this.textRenderer, DETECT_SIZE_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth((StringVisitable)DETECT_SIZE_TEXT), 110, 10526880);
         drawTextWithShadow(matrices, this.textRenderer, SHOW_AIR_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth((StringVisitable)SHOW_AIR_TEXT), 70, 10526880);
      }
      ...
      if (lv == StructureBlockMode.LOAD) {
         ...
         drawTextWithShadow(matrices, this.textRenderer, SHOW_BOUNDING_BOX_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth((StringVisitable)SHOW_BOUNDING_BOX_TEXT), 70, 10526880);
      }
      ...
      drawTextWithShadow(matrices, this.textRenderer, lv.asText(), this.width / 2 - 153, 174, 10526880);
