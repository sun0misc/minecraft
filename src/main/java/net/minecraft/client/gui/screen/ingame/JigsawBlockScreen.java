...
extends Screen {
    ...
    private void updateServer() {
        this.client.getNetworkHandler().sendPacket(new UpdateJigsawC2SPacket(this.jigsaw.getPos(), Identifier.method_60654(this.nameField.getText()), Identifier.method_60654(this.targetField.getText()), Identifier.method_60654(this.poolField.getText()), this.finalStateField.getText(), this.joint, this.parseInt(this.selectionPriorityField.getText()), this.parseInt(this.placementPriorityField.getText())));
    }
    ...
    protected void init() {
        ...
        this.jointRotationButton = this.addDrawableChild(CyclingButtonWidget.builder(JigsawBlockEntity.Joint::asText).values((JigsawBlockEntity.Joint[])JigsawBlockEntity.Joint.values()).initially(this.joint).omitKeyText().build(this.width / 2 + 54, 160, 100, 20, JOINT_LABEL_TEXT, (button, joint) -> {
            this.joint = joint;
        }));
    ...
    private void updateDoneButtonState() {
        boolean bl;
        this.doneButton.active = bl = Identifier.isValid(this.nameField.getText()) && Identifier.isValid(this.targetField.getText()) && Identifier.isValid(this.poolField.getText());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, POOL_TEXT, this.width / 2 - 153, 10, 0xA0A0A0);
        this.poolField.render(context, mouseX, mouseY, delta);
        context.drawTextWithShadow(this.textRenderer, NAME_TEXT, this.width / 2 - 153, 45, 0xA0A0A0);
        ...
        context.drawTextWithShadow(this.textRenderer, FINAL_STATE_TEXT, this.width / 2 - 153, 115, 0xA0A0A0);
        this.finalStateField.render(context, mouseX, mouseY, delta);
        context.drawTextWithShadow(this.textRenderer, SELECTION_PRIORITY_TEXT, this.width / 2 - 153, 150, 0xA0A0A0);
        this.placementPriorityField.render(context, mouseX, mouseY, delta);
        context.drawTextWithShadow(this.textRenderer, PLACEMENT_PRIORITY_TEXT, this.width / 2 - 50, 150, 0xA0A0A0);
        ...
        if (JigsawBlock.getFacing(this.jigsaw.getCachedState()).getAxis().isVertical()) {
            context.drawTextWithShadow(this.textRenderer, JOINT_LABEL_TEXT, this.width / 2 + 53, 150, 0xA0A0A0);
        }
    }
}
