...
implements AutoCloseable {
    ...
    public static WorldIcon forWorld(TextureManager textureManager, String worldName) {
        return new WorldIcon(textureManager, Identifier.method_60656("worlds/" + Util.replaceInvalidChars(worldName, Identifier::isPathCharacterValid) + "/" + String.valueOf(Hashing.sha1().hashUnencodedChars(worldName)) + "/icon"));
    }

    public static WorldIcon forServer(TextureManager textureManager, String serverAddress) {
        return new WorldIcon(textureManager, Identifier.method_60656("servers/" + String.valueOf(Hashing.sha1().hashUnencodedChars(serverAddress)) + "/icon"));
    }
