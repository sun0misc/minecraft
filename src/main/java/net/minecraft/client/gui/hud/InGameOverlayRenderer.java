...
public class InGameOverlayRenderer {
   ...
   private static BlockState getInWallBlockState(PlayerEntity player) {
      ...
      for(int i = 0; i < 8; ++i) {
         ...
         lv.set(d, e, f);
         BlockState lv2 = player.world.getBlockState(lv);
         if (lv2.getRenderType() != BlockRenderType.INVISIBLE && lv2.shouldBlockVision(player.world, lv)) {
            return lv2;
         }
      }
   ...
   private static void renderUnderwaterOverlay(MinecraftClient client, MatrixStack matrices) {
      ...
      float f = LightmapTextureManager.getBrightness(client.player.world.getDimension(), client.player.world.getLightLevel(lv2));
