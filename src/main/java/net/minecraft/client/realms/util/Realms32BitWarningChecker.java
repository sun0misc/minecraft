...
public class Realms32BitWarningChecker {
   ...
   @Nullable
   private CompletableFuture subscriptionRetrievalFuture;
   ...
   private boolean isSubscribedInternal() {
      try {
         return RealmsClient.createRealmsClient(this.client).listWorlds().servers.stream().anyMatch((server) -> {
            return server.ownerUUID != null && !server.expired && server.ownerUUID.equals(this.client.getSession().getUuid());
         });
