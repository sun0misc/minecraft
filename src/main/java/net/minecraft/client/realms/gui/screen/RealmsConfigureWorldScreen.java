...
public class RealmsConfigureWorldScreen extends RealmsScreen {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      if (this.server == null) {
         ...
         drawCenteredTextWithShadow(matrices, this.textRenderer, string, this.width / 2, 24, l);
         int n = Math.min(this.buttonCenter(2, 3) + 80 - 11, this.width / 2 + k / 2 + m / 2 + 10);
         this.drawServerStatus(matrices, n, 7, mouseX, mouseY);
         if (this.isMinigame()) {
            this.textRenderer.draw(matrices, (Text)CURRENT_MINIGAME_TEXT.copy().append(this.server.getMinigameName()), (float)(this.left_x + 80 + 20 + 10), (float)row(13), 16777215);
         }
