...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      drawCenteredTextWithShadow(matrices, this.textRenderer, SUBSCRIPTION_TITLE, this.width / 2, 17, 16777215);
      this.textRenderer.draw(matrices, SUBSCRIPTION_START_LABEL_TEXT, (float)k, (float)row(0), 10526880);
      this.textRenderer.draw(matrices, this.startDate, (float)k, (float)row(1), 16777215);
      if (this.type == Subscription.SubscriptionType.NORMAL) {
         this.textRenderer.draw(matrices, TIME_LEFT_LABEL_TEXT, (float)k, (float)row(3), 10526880);
      } else if (this.type == Subscription.SubscriptionType.RECURRING) {
         this.textRenderer.draw(matrices, DAYS_LEFT_LABEL_TEXT, (float)k, (float)row(3), 10526880);
      }

      this.textRenderer.draw(matrices, this.daysLeft, (float)k, (float)row(4), 16777215);
