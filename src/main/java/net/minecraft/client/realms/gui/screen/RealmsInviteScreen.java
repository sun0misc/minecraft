...
public class RealmsInviteScreen extends RealmsScreen {
   ...
   public void init() {
      this.nameWidget = new TextFieldWidget(this.client.textRenderer, this.width / 2 - 100, row(2), 200, 20, (TextFieldWidget)null, Text.translatable("mco.configure.world.invite.profile.name"));
   ...
   private void onInvite() {
      ...
      if (this.nameWidget.getText() != null && !this.nameWidget.getText().isEmpty()) {
         try {
            RealmsServer lv2 = lv.invite(this.serverData.id, this.nameWidget.getText().trim());
            if (lv2 != null) {
