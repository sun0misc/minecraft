...
extends RealmsScreen {
    ...
    public void init() {
        ...
        ButtonWidget lv = ButtonWidget.builder(Text.translatable(string), button -> {
            if (this.serverData.state == RealmsServer.State.OPEN) {
                this.client.setScreen(RealmsPopups.createInfoPopup(this, Text.translatable("mco.configure.world.close.question.line1"), arg -> this.parent.closeTheWorld()));
            } else {
                this.parent.openTheWorld(false);
            }
        }).dimensions(this.width / 2 - 53, RealmsSettingsScreen.row(0), 106, 20).build();
        this.addDrawableChild(lv);
        this.nameEdit = new TextFieldWidget(this.client.textRenderer, i, RealmsSettingsScreen.row(4), 212, 20, Text.translatable("mco.configure.world.name"));
        this.nameEdit.setMaxLength(32);
        this.nameEdit.setText(this.serverData.getName());
        this.addDrawableChild(this.nameEdit);
        this.descEdit = new TextFieldWidget(this.client.textRenderer, i, RealmsSettingsScreen.row(8), 212, 20, Text.translatable("mco.configure.world.description"));
        this.descEdit.setMaxLength(32);
        this.descEdit.setText(this.serverData.getDescription());
        this.addDrawableChild(this.descEdit);
        ButtonWidget lv2 = this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.buttons.done"), button -> this.save()).dimensions(i - 2, RealmsSettingsScreen.row(12), 106, 20).build());
        this.nameEdit.setChangedListener(name -> {
            arg.active = !StringHelper.isBlank(name);
        });
        this.addDrawableChild(ButtonWidget.builder(ScreenTexts.CANCEL, button -> this.close()).dimensions(this.width / 2 + 2, RealmsSettingsScreen.row(12), 106, 20).build());
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer, WORLD_NAME_TEXT, this.width / 2 - 106, RealmsSettingsScreen.row(3), Colors.WHITE, false);
        context.drawText(this.textRenderer, WORLD_DESCRIPTION_TEXT, this.width / 2 - 106, RealmsSettingsScreen.row(7), Colors.WHITE, false);
    }
