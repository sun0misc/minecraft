...
public class RealmsPlayerScreen extends RealmsScreen {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      drawTexture(matrices, 0, k, 0.0F, 0.0F, this.width, this.height - k, 32, 32);
      RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
      if (this.serverData.players != null) {
         this.textRenderer.draw(matrices, (Text)Text.empty().append(INVITED_TEXT).append(" (").append(Integer.toString(this.serverData.players.size())).append(")"), (float)this.column1_x, (float)row(0), 10526880);
      } else {
   ...
   private class InvitedObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry {
      ...
      public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
         ...
         RealmsPlayerScreen.this.textRenderer.draw(matrices, this.playerInfo.getName(), (float)(RealmsPlayerScreen.this.column1_x + 3 + 12), (float)(y + 1), p);
