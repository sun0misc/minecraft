...
extends AnimalModel<T> {
    ...
    public void animateModel(T arg, float f, float g, float h) {
        ...
        this.bodyPitch = ((BeeEntity)arg).getBodyPitch(h);
        this.stinger.visible = !((BeeEntity)arg).hasStung();
    }
    ...
    public void setAngles(T arg, float f, float g, float h, float i, float j) {
        ...
        boolean bl2 = bl = ((Entity)arg).isOnGround() && ((Entity)arg).getVelocity().lengthSquared() < 1.0E-7;
        if (bl) {
