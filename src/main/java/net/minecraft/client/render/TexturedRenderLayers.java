...
public class TexturedRenderLayers {
    ...
    public static final Map<RegistryKey<DecoratedPotPattern>, SpriteIdentifier> DECORATED_POT_PATTERN_TEXTURES = Registries.DECORATED_POT_PATTERN.streamEntries().collect(Collectors.toMap(RegistryEntry.Reference::registryKey, arg -> TexturedRenderLayers.createDecoratedPotPatternTextureId(((DecoratedPotPattern)arg.value()).assetId())));
    public static final SpriteIdentifier DECORATED_POT_BASE = TexturedRenderLayers.createDecoratedPotPatternTextureId(Identifier.method_60656("decorated_pot_base"));
    public static final SpriteIdentifier DECORATED_POT_SIDE = TexturedRenderLayers.createDecoratedPotPatternTextureId(Identifier.method_60656("decorated_pot_side"));
    public static final SpriteIdentifier[] BED_TEXTURES = (SpriteIdentifier[])Arrays.stream(DyeColor.values()).sorted(Comparator.comparingInt(DyeColor::getId)).map(color -> new SpriteIdentifier(BEDS_ATLAS_TEXTURE, Identifier.method_60656("entity/bed/" + color.getName()))).toArray(SpriteIdentifier[]::new);
