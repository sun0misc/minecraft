...
public class SpawnSettings {
    ...
    public static final MapCodec<SpawnSettings> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(Codec.floatRange(0.0f, 0.9999999f).optionalFieldOf("creature_spawn_probability", Float.valueOf(0.1f)).forGetter(arg -> Float.valueOf(arg.creatureSpawnProbability)), Codec.simpleMap(SpawnGroup.CODEC, Pool.createCodec(SpawnEntry.CODEC).promotePartial((Consumer)Util.addPrefix("Spawn data: ", LOGGER::error)), StringIdentifiable.toKeyable(SpawnGroup.values())).fieldOf("spawners").forGetter(arg -> arg.spawners), Codec.simpleMap(Registries.ENTITY_TYPE.getCodec(), SpawnDensity.CODEC, Registries.ENTITY_TYPE).fieldOf("spawn_costs").forGetter(arg -> arg.spawnCosts)).apply((Applicative<SpawnSettings, ?>)instance, SpawnSettings::new));
    ...
    extends Weighted.Absent {
        ...
        public String toString() {
            return String.valueOf(EntityType.getId(this.type)) + "*(" + this.minGroupSize + "-" + this.maxGroupSize + "):" + String.valueOf(this.getWeight());
        }
    }

    public static class Builder {
        private final Map<SpawnGroup, List<SpawnEntry>> spawners = Stream.of(SpawnGroup.values()).collect(ImmutableMap.toImmutableMap(arg -> arg, arg -> Lists.newArrayList()));
