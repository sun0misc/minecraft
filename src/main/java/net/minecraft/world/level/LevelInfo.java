...
public final class LevelInfo {
   ...
   public static LevelInfo fromDynamic(Dynamic dynamic, DataConfiguration dataConfiguration) {
      ...
      return new LevelInfo(dynamic.get("LevelName").asString(""), lv, dynamic.get("hardcore").asBoolean(false), (Difficulty)dynamic.get("Difficulty").asNumber().map((difficulty) -> {
         return Difficulty.byId(difficulty.byteValue());
      }).result().orElse(Difficulty.NORMAL), dynamic.get("allowCommands").asBoolean(lv == GameMode.CREATIVE), new GameRules(dynamic.get("GameRules")), dataConfiguration);
   }
