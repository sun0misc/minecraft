    ...
    public void forEachInBox(Box box, LazyIterationConsumer<EntityTrackingSection<T>> consumer) {
        ...
        for (int p = j; p <= m; ++p) {
            ...
            while (longIterator.hasNext()) {
                ...
                if (t < k || t > n || u < l || u > o || (lv = (EntityTrackingSection)this.trackingSections.get(s)) == null || lv.isEmpty() || !lv.getStatus().shouldTrack() || !consumer.accept(lv).shouldAbort()) continue;
                return;
            }
        }
    }
