...
public abstract class Feature<FC extends FeatureConfig> {
    ...
    public Feature(Codec<FC> configCodec) {
        this.codec = ((MapCodec)configCodec.fieldOf("config")).xmap(config -> new ConfiguredFeature<FeatureConfig, Feature>(this, (FeatureConfig)config), ConfiguredFeature::config);
    }
    ...
    public static boolean testAdjacentStates(Function<BlockPos, BlockState> posToState, BlockPos pos, Predicate<BlockState> predicate) {
        ...
        for (Direction lv2 : Direction.values()) {
            ...
            if (!predicate.test(posToState.apply(lv))) continue;
            return true;
        }
        return false;
    }
