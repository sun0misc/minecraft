...
public class LootTableReporter {
    ...
    public LootTableReporter makeChild(String name, RegistryKey<?> key) {
        ImmutableCollection set = ((ImmutableSet.Builder)((ImmutableSet.Builder)ImmutableSet.builder().addAll(this.referenceStack)).add(key)).build();
        return new LootTableReporter(this.errorReporter.makeChild(name), this.contextType, this.dataLookup, (Set<RegistryKey<?>>)((Object)set));
    }
