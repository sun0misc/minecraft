...
public record TagKey<T>(RegistryKey<? extends Registry<T>> registry, Identifier id) {
    ...
    public static <T> Codec<TagKey<T>> codec(RegistryKey<? extends Registry<T>> registry) {
        return Codec.STRING.comapFlatMap(string -> string.startsWith("#") ? Identifier.validate(string.substring(1)).map(id -> TagKey.of(registry, id)) : DataResult.error(() -> "Not a tag id"), string -> "#" + String.valueOf(string.id));
    }
    ...
    public String toString() {
        return "TagKey[" + String.valueOf(this.registry.getValue()) + " / " + String.valueOf(this.id) + "]";
    }
}
