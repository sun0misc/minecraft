...
public class BuiltinRegistries {
    ...
    public static void validate(RegistryEntryLookup<PlacedFeature> placedFeatureLookup, RegistryWrapper<Biome> biomeLookup) {
        biomeLookup.streamEntries().forEach(biome -> {
            ...
            list.stream().flatMap(RegistryEntryList::stream).forEach(placedFeature -> placedFeature.getKeyOrValue().ifLeft(key -> {
                RegistryEntry.Reference lv = placedFeatureLookup.getOrThrow((RegistryKey<PlacedFeature>)key);
                if (!BuiltinRegistries.hasBiomePlacementModifier((PlacedFeature)lv.value())) {
                    Util.error("Placed feature " + String.valueOf(key.getValue()) + " in biome " + String.valueOf(lv) + " is missing BiomeFilter.biome()");
                }
            }).ifRight(value -> {
                if (!BuiltinRegistries.hasBiomePlacementModifier(value)) {
                    Util.error("Placed inline feature in biome " + String.valueOf(biome) + " is missing BiomeFilter.biome()");
                }
            }));
        });
    }
    ...
    public static RegistryWrapper.WrapperLookup createWrapperLookup() {
        ...
        RegistryWrapper.WrapperLookup lv2 = REGISTRY_BUILDER.createWrapperLookup(lv);
        BuiltinRegistries.validate(lv2);
        return lv2;
    }
}
