      ...
      biomeLookup.streamEntries().forEach((biome) -> {
         ...
         list.stream().flatMap(RegistryEntryList::stream).forEach((placedFeature) -> {
            placedFeature.getKeyOrValue().ifLeft((key) -> {
               RegistryEntry.Reference lvx = placedFeatureLookup.getOrThrow(key);
               if (!hasBiomePlacementModifier((PlacedFeature)lvx.value())) {
                  Identifier var10000 = key.getValue();
                  Util.error("Placed feature " + var10000 + " in biome " + lv + " is missing BiomeFilter.biome()");
               }
